...
cfg.Add(
  BoolVariable(
    'NSIS_CONFIG_EXTERNAL_FILE_SUPPORT',
    'enables support for making installers that can use an external file for storing install data. Useful for installers that exceed the 4GB limit of standard "all-in-one" installers.',
    'yes'
  )
)
...
cfg.Add(
  BoolVariable(
    'NSIS_CONFIG_LARGE_FILE_SUPPORT',
    'enables support for making installers that allows source files bigger than 2GB to be added to the installer.',
    'yes'
  )
)
...
AddBoolDefine('NSIS_CONFIG_EXTERNAL_FILE_SUPPORT')
AddBoolDefine('NSIS_CONFIG_LARGE_FILE_SUPPORT')
