  ...
  if (hInet)
  {
    ...
    if (hConn)
    {
      ...
      if (!gle)
      {
        ...
        {
        case '0':
          msg = "目前还没有可用的NSIS更新版本.";
          break;
        case '1':
        case '2':
          if (cbRecv > 2 && '|' == response[1])
          {
            const char *reltypestr = '1' == response[0] ? "这是" : "发布预览版";
            wsprintfA(buf, "NSIS %.50s 可用. 你想现在就下载 %s 吗?", response+2, reltypestr);
...
void CheckForUpdate()
{
  ...
  {
    MessageBoxA(g_sdata.hwnd, "无法检测每日构建版（测试版）的新版本。要更新，请下载新的每日构建版（测试版）.", mbtitle, MB_OK|MB_ICONSTOP);
    return;
  }
