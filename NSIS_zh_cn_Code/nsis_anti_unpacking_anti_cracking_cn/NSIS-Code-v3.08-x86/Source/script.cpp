...
LANGID CEXEBuild::ParseLangIdParameter(const LineParser&line, int token)
{
  ...
  if (!succ)
    warning_fl(DW_BAD_LANGID, _T("\"%") NPRIs _T("\" 错误的语言 id, 使用语言 id %u!"), line.gettoken_str(token), lid);
  return lid;
}
...
int CEXEBuild::process_script(NIStream&Strm, const TCHAR *filename)
{
  ...
  {
    ERROR_MSG(_T("错误 (处理脚本): write_output 已经调用, 无法继续\n"));
    return PS_ERROR;
  }
  ...
  {
    ERROR_MSG(_T("!if[macro][n]def: 在 EOF 处打开 - 需要 !endif\n"));
    return PS_ERROR;
  }
...
parse_again:
  ...
  if (tkid == TOK_P_ELSE)
  {
    ...
    {
      ERROR_MSG(_T("!else: 当前 if 中已经有了 else\n"));
      return PS_ERROR;
    }
      ...
      tkid == TOK_P_IF)
  {
    ...
    if (tkid == TOK_P_IF) {
      ...
      if (line.getnumtokens() == 2)
        ...
        if (!cnv1 || !cnv2) {
          warning_fl(DW_PARSE_BADNUMBER, _T("无效的数字: \"%") NPRIs _T("\""), line.gettoken_str(!cnv1 ? 1 : 3));
        }
      }
      else PRINTHELPEX(cmdnam)
...
int CEXEBuild::LoadLicenseFile(const TCHAR *file, TCHAR** pdata, const TCHAR *cmdname, WORD AnsiCP) // caller must free *pdata, even on error result
{
  ...
  *((TCHAR*)((char*)data+cbTotalData-sizeof(TCHAR)))=_T('\0');
  ...
  {
l_errwcconv:
    ERROR_MSG(_T("%") NPRIs _T(": 获取文件数据失败!\n"),cmdname);
    return PS_ERROR;
  }
...
int CEXEBuild::process_jump(LineParser &line, int wt, int *offs)
{
  ...
  else
  {
    ...
    {
      ERROR_MSG(_T("错误: Goto 跳转目标以 '+' 或 '-' 开头时后面必须跟随非零整数 (相对跳转)\n"));
      return 1;
    }
...
int CEXEBuild::doCommand(int which_token, LineParser &line)
{
  ...
  switch (which_token)
  {
    ...
    case TOK_P_MACROEND:
    return (ERROR_MSG(_T("!macroend: 该处没有宏打开.\n")), PS_ERROR);
    ...
    case TOK_PAGE:
      {
        ...
        if (!k) {
          ERROR_MSG(_T("错误: 自定义页面, NSIS_SUPPORT_CODECALLBACKS 没有定义.\n"));
          return PS_ERROR;
        }
    ...
    return PS_OK;
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_CODECALLBACKS 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_SUPPORT_CODECALLBACKS
#else
    case TOK_PAGE:
    ...
    case TOK_PAGEEXEND:
    case TOK_PAGECALLBACKS:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_VISIBLE_SUPPORT 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_LANGSTRINGUP:
      SCRIPT_MSG(_T("错误: LangStringUP 已废弃, 不会再有非处理的字串. 请使用 LangString.\n"));
    return PS_ERROR;
    case TOK_LICENSELANGSTRING:
    {
      ...
      if (build_header.flags&(CH_FLAGS_SILENT|CH_FLAGS_SILENT_LOG))
        warning_fl(DW_LANGSTRING_SILENTLICENSE, _T("%") NPRIs _T(": SilentInstall 启用, 浪费空间"), cmdnam);
#endif
      TCHAR *name = line.gettoken_str(1);
      LANGID lang = ParseLangIdParameter(line, 2);
      ...
      if (ret == PS_WARNING)
        warning_fl(DW_LANGSTRING_MULTISETWASTE, _T("%") NPRIs _T(" \"%") NPRIs _T("\" 对 %d 重复设置, 浪费空间"), cmdnam, name, lang);
      else if (ret == PS_ERROR)
      {
        ERROR_MSG(_T("错误: 不能设置 %") NPRIs _T(" \"%") NPRIs _T("\"!\n"), cmdnam, name);
        return PS_ERROR;
      }
        ...
        if (err) {
          switch (err) {
            case -1:
              ...
              break;
            case -2:
              ERROR_MSG(_T("错误: 无效的位图文件 - 存在错误或不是位图\n"));
              break;
            case -3:
              ERROR_MSG(_T("错误: 位图大小不是 96x16\n"));
              break;
            case -4:
              ERROR_MSG(_T("错误: 位图位数高于 8bpp\n"));
              break;
          }
          return PS_ERROR;
        }
...
#else
    case TOK_CHECKBITMAP:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_COMPONENTPAGE 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
...
#ifdef NSIS_CONFIG_VISIBLE_SUPPORT
      {
        if (!cur_page) {
          ...
          if (cur_page_type != PAGE_DIRECTORY) {
            ERROR_MSG(_T("错误: DirText 只能在 PageEx directory 页面中使用.\n"));
            return PS_ERROR;
          }
    ...
    case TOK_INSTTYPE:
      {
        ...
        else
        {
          ...
          {
            ERROR_MSG(_T("InstType: 安装类型总数不能超过 %d 个. 现已指定 %d 个\n"), NSIS_MAX_INST_TYPES, NSIS_MAX_INST_TYPES + 1);
            return PS_ERROR;
          }
          else
          {
    ...
    case TOK_LICENSEDATA:
    case TOK_LICENSEBKCOLOR:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_LICENSEPAGE 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_SILENTINST:
    {
      ...
      if (k == 2)
      {
        ERROR_MSG(_T("SilentInstall: 已指定 silentlog , 但编译时没有 (在 NSIS_CONFIG_LOG 处) 启用记录功能\n"));
        return PS_ERROR;
      }
    ...
    return PS_OK;
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_UNINSTALL_SUPPORT 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif
    case TOK_IFSILENT:
    ...
    case TOK_IFSILENT:
    case TOK_SETSILENT:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_SILENT_SUPPORT 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
...
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    {
      ...
      if (which_token == TOK_SETEXOUTFILE)
      {
        ...
        warning_fl(DW_PARSE_SETEXOUTFILE_DEPRECATED, _T("SetExOutFile: 重命名输入文件模式 auto|aio|data|stub"));
      }
      else
      {
        arg = line.gettoken_enum(1,_T("auto\0aio\0data\0stub\0")) - 1;        
      }
      switch (arg)
      {
        default:
      ...
      if (arg < OFM_AUTO) PRINTHELP()
#ifdef NSIS_CONFIG_COMPRESSION_SUPPORT
      if (arg != OFM_AIO && build_compress_whole)
      {
        ERROR_MSG(_T("错误: %") NPRIs _T(" %") NPRIs _T(": 固实压缩不能和外部文件一起使用.\n"), line.gettoken_str(0), str);
        return PS_ERROR;
      }
      if (arg != build_external_file && build_compressor_set)
      {
        ERROR_MSG(_T("Error: %") NPRIs _T(" %") NPRIs _T(": 添加文件后无法更改输出文件模式.\n"), line.gettoken_str(0), str);
        return PS_ERROR;
      }
#endif
      build_external_file = arg;
      build_datablock.setlimit(build_external_file != OFM_AIO ? (IGrowBuf64::size_type)0x7fffffffffffffff : (IGrowBuf64::size_type)0xffffffff);
      SCRIPT_MSG(_T("输出文件模式: %") NPRIs _T("\n"),str);
    }
    return PS_OK;
#else//!NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的, NSIS_CONFIG_EXTERNAL_FILE_SUPPORT 没有被定义.\n"), line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_INSTPROGRESSFLAGS:
      {
        ...
        try {
          ...
          ERROR_MSG(_T("设置 smooth 进度条出错: %") NPRIs _T("\n"), CtoTStrParam(err.what()));
          return PS_ERROR;
        }
    ...
    case TOK_SHOWDETAILSUNINST:
      ...
      ERROR_MSG(_T("错误: ShowUninstDetails 已指定 但 NSIS_CONFIG_UNINSTALL_SUPPORT 未定义\n"));
      return PS_ERROR;
#endif
    ...
    case TOK_BGFONT:
      ...
      ERROR_MSG(_T("错误: BGFont 已指定 但 NSIS_SUPPORT_BGBG 未定义\n"));
      return PS_ERROR;
    ...
    case TOK_BGGRADIENT:
#ifndef NSIS_SUPPORT_BGBG
      ERROR_MSG(_T("错误: BGGradient 已指定 但 NSIS_SUPPORT_BGBG 未定义\n"));
      return PS_ERROR;
#else // NSIS_SUPPORT_BGBG
      if (line.getnumtokens()==1)
      {
    ...
    case TOK_CHANGEUI:
      try {
        ...
        ERROR_MSG(_T("更换 UI 出错: %") NPRIs _T("\n"), CtoTStrParam(err.what()));
        return PS_ERROR;
      }
    return PS_OK;
...
#ifdef NSIS_CONFIG_COMPRESSION_SUPPORT
    {
      if (build_compressor_set) {
        ERROR_MSG(_T("错误: 无法在数据压缩或文件头部更改完成后, 再更改压缩器!!\n"));
        return PS_ERROR;
      }
      if (build_compressor_final)
      {
        warning_fl(DW_COMP_FINAL, _T("是否忽略 SetCompressor 取决于之前的 /FINAL 开关调用"));
        return PS_OK;
      }
      ...
      while (line.gettoken_str(a)[0] == _T('/'))
      {
        ...
        else if (!_tcsicmp(line.gettoken_str(a),_T("/SOLID")))
        {
          ...
          if (build_external_file <= OFM_AUTO)
            warning_fl(DW_COMP_WHOLE_IGNORE, _T("设置压缩：超出文件自动模式，忽略/固体标志。"));
          else if (build_external_file == OFM_DATA)
            warning_fl(DW_COMP_WHOLE_IGNORE, _T("设置压缩：超出文件模式的数据而忽略/固体标志"));
          else if (build_external_file == OFM_STUB)
            warning_fl(DW_COMP_WHOLE_IGNORE, _T("设置压缩：忽略/固体标志"));
          else
#endif
          {
      ...
      switch (k) {
        ...
        case 3: compressor = &lz4_compressor; break;
        default: PRINTHELP();
      }
    ...
    return PS_OK;
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_COMPRESSION_SUPPORT 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_WRITEUNINSTALLER:
    {
      if (uninstall_mode)
      {
        ERROR_MSG(_T("WriteUninstaller 仅在安装程序中有效, 而不是卸载程序.\n"));
        PRINTHELP()
      }
    ...
    case TOK_SECTION:
    {
      ...
      {
        ERROR_MSG(_T("错误: 卸载函数没有在 NSIS_CONFIG_UNINSTALL_SUPPORT 中\n"));
        return PS_ERROR;
      }
#endif
    ...
    case TOK_FUNCTION:
      ...
      {
        ERROR_MSG(_T("Function: 函数名称不能以 : 或 / 开头.\n"));
        PRINTHELP()
      }
    ...
    case TOK_SETCOMPRESS:
      ...
      if (build_compress==0 && build_compress_whole)
        warning_fl(DW_COMP_WHOLE_IGNORE_OFF, _T("数据已经在 整个压缩 模式中压缩. 跳过."));
    ...
    case TOK_FILEBUFSIZE:
      ...
      {
        ERROR_MSG(_T("错误: FileBufSize: 无效的 buffer 值 -- %d\n"),build_filebuflen);
        return PS_ERROR;
      }
    ...
    case TOK_SETCOMPRESSIONLEVEL:
    {
      if (compressor == &lzma_compressor)
        warning_fl(DW_COMP_LEVEL_IGNORE, _T("SetCompressionLevel: 压缩器设定为 LZMA. 跳过."));
      if (build_compressor_set && build_compress_whole)
        warning_fl(DW_COMP_LEVEL_IGNORE, _T("SetCompressionLevel: 数据已经在 整个压缩 模式中压缩. 跳过."));
    ...
    {
      if (compressor != &lzma_compressor)
        warning_fl(DW_COMP_DICT_IGNORE, _T("SetCompressorDictSize: 压缩器设定为 LZMA. 跳过."));
      if (build_compressor_set && build_compress_whole)
        warning_fl(DW_COMP_DICT_IGNORE, _T("SetCompressorDictSize: 数据已经在 整体压缩 模式中压缩. 跳过."));
    ...
    return PS_OK;
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_FILE 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif
    case TOK_BRANDINGTEXT:
      {
        ...
        if (trim)
        {
          ERROR_MSG(_T("错误︰ BrandingText /TRIM* 禁用非 Win32 平台.\n"));
          return PS_ERROR;
        }
#endif
    ...
    case TOK_COMPLETEDTEXT:
      {
        if (!cur_page) {
          ...
          if (cur_page_type != PAGE_INSTFILES) {
            ERROR_MSG(_T("错误: CompletedText 只能在 PageEx instfiles 页面中使用.\n"));
            return PS_ERROR;
          }
    ...
    case TOK_CALL:
      ...
      {
        ERROR_MSG(_T("Call 在非卸载区段中不能与以 \"un.\" 开头的函数(卸载函数)一起使用.\n"));
        PRINTHELP()
      }
      if (!uninstall_mode && !_tcsnicmp(line.gettoken_str(1),_T("un."),3))
      {
        ERROR_MSG(_T("Call 在卸载区段中必须与以 \"un.\" 开头的函数(卸载函数)一起使用.\n"));
        PRINTHELP()
      }
#endif
      ent.which=EW_CALL;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_EXECUTE 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_SHELLEXECUTE 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_RENAME:
#ifdef NSIS_SUPPORT_RENAME
      {
        ...
        if (!_tcsicmp(line.gettoken_str(1),_T("/REBOOTOK")))
        {
          ...
          ERROR_MSG(_T("错误: /REBOOTOK 已指定, NSIS_SUPPORT_MOVEONREBOOT 未定义\n"));
          PRINTHELP()
#endif
        }
        else if (line.gettoken_str(1)[0]==_T('/'))
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_RENAME 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_MESSAGEBOX 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
...
#ifdef NSIS_SUPPORT_CREATESHORTCUT
    {
      ...
      if (line.getnumtokens() > 6 && *line.gettoken_str(6))
      {
        ...
        {
          ERROR_MSG(_T("CreateShortcut: 未知的显示方式 \"%") NPRIs _T("\"\n"),line.gettoken_str(6));
          PRINTHELPEX(cmdnam)
        }
        ...
        if (*s)
        {
          ...
          if ((s[0] == _T('F')) && (s[1] >= _T('1') && s[1] <= _T('9')))
          {
            ...
            if (_ttoi(s+1) < 1 || _ttoi(s+1) > 24)
              warning_fl(DW_PARSE_LNK_HK, _T("CreateShortcut: F-key \"%") NPRIs _T("\" 超出范围"),s);
          }
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_CREATESHORTCUT 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_HIDEWINDOW:
    case TOK_ENABLEWINDOW:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_ENHANCEDUI_SUPPORT 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_CONFIG_ENHANCEDUI_SUPPORT
    ...
    case TOK_HIDEWINDOW:
    case TOK_BRINGTOFRONT:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_HWNDS 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_DELETE 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_RMDIR 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_STUBFILE:
#ifdef NSIS_SUPPORT_FILE
      {
        ...
        if (which_token == TOK_FILE && !_tcsicmp(line.gettoken_str(a),_T("/a")))
        {
          ...
          attrib=1;
#else
          warning_fl(DW_UNSUPP_STORE_FILE_ATT, _T("%") NPRIs _T("对于非Win32平台，文件/ a 被禁用."),which_token_str);
#endif
          a++;
        }
        ...
        else if ((which_token == TOK_FILE || which_token == TOK_STUBFILE) && !_tcsnicmp(line.gettoken_str(a),_T("/oname="),7))
        {
          ...
          if (on[0]==_T('"'))
          {
            ERROR_MSG(_T("%") NPRIs _T("文件: 输出 名称不能以引号开头, 请用 \"/oname=name (带空格)\".\n"),which_token_str,line.gettoken_str(a));
            PRINTHELP();
          }
          ...
          if (!tf)
          {
            if (fatal)
            {
              ERROR_MSG(_T("%") NPRIs _T("文件: \"%") NPRIs _T("\" -> 未找到文件.\n"),which_token_str,line.gettoken_str(a));
              PRINTHELP()
            }
            else
            {
              warning_fl(DW_FILE_NONFATAL_NOT_FOUND, _T("%") NPRIs _T("文件: \"%") NPRIs _T("\" -> 未找到文件"),which_token_str,line.gettoken_str(a));
        ...
        while (a < line.getnumtokens())
        {
          ...
          if (!tf)
          {
            ...
            {
              warning_fl(DW_FILE_NONFATAL_NOT_FOUND, _T("%") NPRIs _T("文件: \"%") NPRIs _T("\" -> 未找到文件."),which_token_str,t);
            }
          }
        }
      }
...
#endif //~ NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的, NSIS_SUPPORT_FILE 没有被定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_SUPPORT_FILE
...
#else
    case TOK_COPYFILES:
      ERROR_MSG(_T("Error: %") NPRIs _T(" 指定的, NSIS_SUPPORT_COPYFILES 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_GETDLLVERSIONLOCAL:
      {
        ...
        {
          ERROR_MSG(_T("%") NPRIs _T(": 读取版本信息时出错 \"%") NPRIs _T("\"\n"),cmdname,line.gettoken_str(1));
          return PS_ERROR;
        }
        ent.which=EW_ASSIGNVAR;
    ...
    case TOK_GETFILETIMELOCAL:
      {
        ...
        if (!flag)
        {
          ERROR_MSG(_T("GetFileTimeLocal: 读取日期出错 \"%") NPRIs _T("\"\n"),line.gettoken_str(1));
          return PS_ERROR;
        }
#else
        struct stat st;
    ...
    case TOK_STRCMP:
    case TOK_STRCMPS:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_STROPTS 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_WRITEINISTR:
    case TOK_READINISTR:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_INIFILES 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_GETFULLPATHNAME:
    case TOK_GETKNOWNFOLDERPATH:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_FNUTIL 未定义.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_GETDLLVERSION 未定义.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_GETFILETIME 未定义.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    case TOK_INTFMT: case TOK_INT64FMT:
    case TOK_INTCMP: case TOK_INTCMPU: case TOK_INT64CMP: case TOK_INT64CMPU: case TOK_INTPTRCMP: case TOK_INTPTRCMPU:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_INTOPTS 未定义.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    case TOK_WRITEREGDWORD:
      {
        ...
        ent.offsets[1]=add_string(line.gettoken_str(2));
        if (line.gettoken_str(2)[0] == _T('\\'))
          warning_fl(DW_PARSE_REGPATHPREFIX, _T("%") NPRIs _T(": 注册表路径名称以 \'\\\', 开头时可能会出错"),cmdname);
        ent.offsets[2]=add_string(line.gettoken_str(3));
...
#endif
          IGrowBuf64::size_type offset = add_db_data(data,data_len,&crc);
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
          set_datablock_mode(0);
#endif
          if (offset < 0) return PS_ERROR;
          DB_OFFSET off;
          off.quad_part = offset;
          ent.offsets[3]=off.low_part;
          ent.offsets[4]=off.high_part;
    ...
    case TOK_ENUMREGKEY:
    case TOK_ENUMREGVAL:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_REGISTRYFUNCTIONS 未定义.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    case TOK_PUSH:
    case TOK_EXCH:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_STACK 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_EXPANDENVSTRS:
    case TOK_READENVSTR:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_ENVIRONMENT 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_FINDNEXT:
    case TOK_FINDFIRST:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_FINDFIRST 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_FILEREADUTF16LE:
      if (!build_unicode)
      {
        ERROR_MSG(_T("错误: %") NPRIs _T(" Unicode只支持Unicode编码的安装程序\n"),  line.gettoken_str(0));
        return PS_ERROR;
      }
      ent.which=EW_FGETWS;
      ent.offsets[0]=GetUserVarIndex(line, 1); // file handle
    ...
    case TOK_FILEWRITEWORD:
      if (!build_unicode)
      {
        ERROR_MSG(_T("错误: %") NPRIs _T(" 仅在构建Unicode安装程序时可用\n"),  line.gettoken_str(0));
        return PS_ERROR;
      }
      ent.which=EW_FPUTWS;
      ent.offsets[0]=GetUserVarIndex(line, 1); // file handle
    ...
    case TOK_FILEWRITEWORD:
#endif
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_FILEFUNCTIONS 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_IFREBOOTFLAG:
    case TOK_SETREBOOTFLAG:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_REBOOT 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_LOGSET:
    case TOK_LOGTEXT:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_LOG 未定义.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_CONFIG_LOG
#ifdef NSIS_CONFIG_COMPONENTPAGE
    ...
    case TOK_SETBRANDINGIMAGE:
    case TOK_LOADANDSETIMAGE:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_ENHANCEDUI_SUPPORT 未定.\n"),line.gettoken_str(0));
      return PS_ERROR;
#endif //~ NSIS_CONFIG_ENHANCEDUI_SUPPORT
    case TOK_DEFVAR:
    {
    ...
    case TOK_VI_ADDKEY:
    {
      ...
      else
      {
        ...
        {
          ERROR_MSG(_T("%") NPRIs _T(": \"%") NPRIs _T("\" 错误的语言文件!\n"),line.gettoken_str(0), line.gettoken_str(1));
          return PS_ERROR;
        }
      ...
      // It is ok for us to use rVersionInfo as storage since VIProductVersion is required by VIAddVersionKey 
      {
        ...
        if (settingFileVer)
        {
          if (!validInput) 
          {
            ERROR_MSG(_T(" ProductVersion 格式错误 %") NPRIs _T(" 正确的格式为 X.X.X.X\n"),line.gettoken_str(0));
            return PS_ERROR;
          }
    ...
    case TOK_VI_SETPRODUCTVERSION:
    case TOK_VI_SETFILEVERSION:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_SUPPORT_VERSION_INFO 未定义.\n"),line.gettoken_str(0));
      return PS_ERROR;
    ...
    case TOK__PLUGINCOMMAND:
    {
      ...
      {
        ERROR_MSG(_T("插件命令 %") NPRIs _T(" 冲突与另一个目录中的一个插件!\n"),command.c_str());
        return PS_ERROR;
      }

      tstring dllName = get_file_name(dllPath);
      IGrowBuf64::size_type data_handle = m_pPlugins->GetDllDataHandle(!!uninstall_mode, command);
    ...
    case TOK__PLUGINCOMMAND:
    case TOK_INITPLUGINSDIR:
      ERROR_MSG(_T("错误: %") NPRIs _T(" 指定的 NSIS_CONFIG_PLUGIN_SUPPORT 未定义.\n"),line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_CONFIG_PLUGIN_SUPPORT
...
#ifdef NSIS_SUPPORT_FILE
int CEXEBuild::do_add_file(const TCHAR *lgss, int attrib, int recurse, int *total_files, const TCHAR *name_override, int generatecode, IGrowBuf64::size_type *data_handle, crc32_t *pcrc, const set<tstring>& excluded, const tstring& basedir, bool dir_created)
{
...
int CEXEBuild::add_file(const tstring& dir, const tstring& file, int attrib, const TCHAR *name_override, int generatecode, IGrowBuf64::size_type *data_handle, crc32_t *pcrc, const TCHAR *which_token_str) {
  ...
  if (hFile == INVALID_HANDLE_VALUE)
  {
    ERROR_MSG(_T("%") NPRIs _T("文件: 无法打开文件 \"%") NPRIs _T("\"\n"),which_token_str,newfn);
    return PS_ERROR;
  }
  MANAGE_WITH(hFile, CloseHandle);

  DWORD high, ret = 0;
  len.low_part = GetFileSize(hFile, &high);
  if (INVALID_FILE_SIZE == len.low_part || GetLastError()) ret++;
  len.high_part = high;
  if (!mmap.setfile(hFile, len.quad_part) && ret)
  {
    ERROR_MSG(_T("%") NPRIs _T("文件: 创建 mmap 失败 \"%") NPRIs _T("\"\n"),which_token_str,newfn);
    return PS_ERROR;
  }
#else // !_WIN32
  ...
  struct stat s;
  if (fstat(fd, &s)) {
    ERROR_MSG(_T("%") NPRIs _T("文件: 声明失败 \"%") NPRIs _T("\"\n"),which_token_str,newfn);
    return PS_ERROR;
  }
  len.quad_part = s.st_size;
  if (len.quad_part && !mmap.setfile(fd, len.quad_part))
  {
    ERROR_MSG(_T("%") NPRIs _T("文件: 创建 mmap 失败 \"%") NPRIs _T("\"\n"),which_token_str,newfn);
    return PS_ERROR;
  }
  ...
  if (generatecode&1)
    section_add_size_kb((int)((len.quad_part+1023)/1024));
  ...
  fflush(stdout);
  TCHAR buf[1024];
  IGrowBuf64::size_type last_build_datablock_used=getcurdbsize();
  entry ent={0,};
  if (generatecode)
  {
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
  ...
  crc32_t crc = 0;
  IGrowBuf64::size_type db_offset = add_db_data(&mmap, &crc);
  mmap.clear();
  if (db_offset < 0) return PS_ERROR;
  DB_OFFSET off;
  off.quad_part = db_offset;
  ...
  if (data_handle)
    *data_handle = db_offset;

  {
    IGrowBuf64::size_type s=getcurdbsize()-last_build_datablock_used;
    if (s) s-=FIRST_INT_SIZEOF;
    if (s != len.quad_part)
    {
      FriendlySize us(len.quad_part, GFSF_BYTESIFPOSSIBLE); // uncompressed installer size
      FriendlySize cs(s, GFSF_BYTESIFPOSSIBLE | (us.UInt()==(len.quad_part <= 0xffffffff ? len.low_part : 0) ? GFSF_HIDEBYTESCALE : 0)); // compressed installer size
      SCRIPT_MSG(_T(" %u%") NPRIs _T(" / %u%") NPRIs _T("\n"),
        cs.UInt(),(!_tcsicmp(cs.Scale(),us.Scale())?_T(""):cs.Scale()),us.UInt(),us.Scale());
    }
    else
    {
      FriendlySize us(len.quad_part, GFSF_BYTESIFPOSSIBLE); // uncompressed installer size
  ...
  if (generatecode)
  {
    if (build_datesave || build_overwrite>=0x3 /*ifnewer or ifdiff*/)
    {
      ...
      {
        ERROR_MSG(_T("%") NPRIs _T("文件: 读取日期出错 \"%") NPRIs _T("\"\n"),which_token_str,newfn);
        return PS_ERROR;
      }
    }
    else
