    ...
    case EW_VERIFYEXTERNALFILE:
      {
        ...
        else if (g_is_stub_installer)
        {
          ...
          if (!validpathspec(buf0))
          {
            ...
            mystrcpy(buf1, trimextension(buf1));
            wsprintf(buf0, _T("%s.nsisbin"), buf1);
          }
          validate_filename(buf0);
          g_dbex_hFile = myOpenFile(buf0, GENERIC_READ, OPEN_EXISTING);
          if (g_dbex_hFile != INVALID_HANDLE_VALUE)
            exec_error--;
        }
      }
    break;
