...
IGrowBuf64::size_type Plugins::GetDllDataHandle(bool uninst, const tstring& command) const
{
  const tstring dllname = GetDllName(command);
  if (uninst)
    return get_paired_value(m_dllname_to_unst_datahandle, dllname, (IGrowBuf64::size_type)-1);
  else
    return get_paired_value(m_dllname_to_inst_datahandle, dllname, (IGrowBuf64::size_type)-1);
}
...
void Plugins::SetDllDataHandle(bool uninst, tstring&canoniccmd, IGrowBuf64::size_type dataHandle, crc32_t crc)
{
...
{
  IGrowBuf64::size_type h = GetDllDataHandle(false, dllnamelowercase);
