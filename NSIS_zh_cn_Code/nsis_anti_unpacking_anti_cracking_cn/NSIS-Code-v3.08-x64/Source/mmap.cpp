...
#ifdef _WIN32
int MMapFile::setfile(HANDLE hFile, UINT64 dwSize)
#else
int MMapFile::setfile(int hFile, UINT64 dwSize)
#endif
{
  clear();
...
#ifdef _WIN32
  DB_OFFSET off;
  off.quad_part = m_iSize;
  m_hFileMap = CreateFileMapping(m_hFile, NULL, PAGE_READONLY, off.high_part, off.low_part, NULL);
...
void MMapFile::resize(IGrowBuf64::size_type newsize)
{
  release();
...
IGrowBuf64::size_type MMapFile::getsize() const
{
  return m_iSize;
}

void *MMapFile::get(IGrowBuf64::size_type offset, IGrowBuf64::size_type size) const
{
  return get(offset, &size);
}

void *MMapFile::get(IGrowBuf64::size_type offset, IGrowBuf64::size_type *sizep) const
{
  if (!sizep)
    return NULL;
  ...
  IGrowBuf64::size_type size = *sizep;
  ...
  // fix offset
  IGrowBuf64::size_type alignedoffset = offset - (offset % m_iAllocationGranularity);
  size += offset - alignedoffset;

#ifdef _WIN32
  DB_OFFSET off;
  off.quad_part = alignedoffset;
  m_pView = MapViewOfFile(m_hFileMap, m_bReadOnly ? FILE_MAP_READ : FILE_MAP_WRITE, off.high_part, off.low_part, (SIZE_T)size);
#else
...
void *MMapFile::getmore(IGrowBuf64::size_type offset, IGrowBuf64::size_type size) const
{
  void *pView;
  void *pViewBackup = m_pView;
#ifndef _WIN32
  IGrowBuf64::size_type iMappedSizeBackup = m_iMappedSize;
#endif
  m_pView = 0;
...
void MMapFake::set(const char *pMem, FIRST_INT_TYPE iSize)
{
  m_pMem = pMem;
  m_iSize = iSize;
}

IGrowBuf64::size_type MMapFake::getsize() const
{
  return m_iSize;
}

void *MMapFake::get(IGrowBuf64::size_type offset, IGrowBuf64::size_type size) const
{
  return get(offset, &size);
}
...
void *MMapFake::getmore(IGrowBuf64::size_type offset, IGrowBuf64::size_type size) const
{
  return get(offset, size);
}

void MMapFake::resize(IGrowBuf64::size_type n) {}
...
void MMapBuf::setlimit(IGrowBuf64::size_type limit)
{
  cur_limit = limit;
}

IGrowBuf64::size_type MMapBuf::getlimit()
{
  return cur_limit;
}

IGrowBuf64::size_type MMapBuf::add(const void *data, int len) 
{ 
  if (len <= 0) return 0;
  if (getlen() > INT_MAX) return 0;
  resize(getlen() + len);
...
void MMapBuf::resize(IGrowBuf64::size_type newlen)
{
  ...
  if (newlen > m_alloc)
  {
    ...
    if (m_alloc < 0) // we've hit a signed integer overflow
        m_alloc = 0x7fffffffffffffff;

    m_fm.resize(m_alloc);

    if (m_gb.getlen())
    {
      memcpy(m_fm.get(0, m_gb.getlen()), m_gb.get(), (size_t)m_gb.getlen());
      m_fm.flush((size_t)m_gb.getlen());
      m_fm.release();
      m_gb.resize(0);
    }
  }
...
IGrowBuf64::size_type MMapBuf::getsize() const
{
  if (m_gb_u)
    return m_fm.getsize();
  return m_gb.getlen();
...
IGrowBuf64::size_type MMapBuf::getlen() const
{
  if (m_gb_u)
    return m_used;
  return m_gb.getlen();
...
void *MMapBuf::get(IGrowBuf64::size_type offset, IGrowBuf64::size_type *sizep) const
{
  if (!sizep)
    return NULL;
  IGrowBuf64::size_type size = *sizep;
...
void *MMapBuf::get(IGrowBuf64::size_type offset, IGrowBuf64::size_type size) const
{
  if (m_gb_u)
...
void *MMapBuf::getmore(IGrowBuf64::size_type offset, IGrowBuf64::size_type size) const
{
  if (m_gb_u)
