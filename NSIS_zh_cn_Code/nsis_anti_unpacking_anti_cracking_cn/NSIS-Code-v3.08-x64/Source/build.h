...
typedef enum {
  ...
  DW_PARSE_SETEXOUTFILE_DEPRECATED = 7950,
...
class CEXEBuild {
  ...
  // Added by Sunil Kamath 11 June 2003
    ...
    int do_add_file(const TCHAR *lgss, int attrib, int recurse, int *total_files, const TCHAR
      *name_override=0, int generatecode=1, IGrowBuf64::size_type *data_handle=0, crc32_t *pcrc=0,
      const std::set<tstring>& excluded=std::set<tstring>(),
      const tstring& basedir=tstring(_T("")), bool dir_created=false);
    int add_file(const tstring& dir, const tstring& file, int attrib, const TCHAR *name_override, int
      generatecode, IGrowBuf64::size_type *data_handle, crc32_t *pcrc, const TCHAR *which_token_str);
    ...
    IGrowBuf64::size_type getcurdbsize();
    IGrowBuf64::size_type getcurdblimit();
    ...
    int add_entry_direct(int which, int o0=0, int o1=0, int o2=0, int o3=0, int o4=0, int o5=0, int o6=0, int o7=0);
    IGrowBuf64::size_type add_db_data(IMMap *map, crc32_t *ex_crc); // returns offset
    IGrowBuf64::size_type add_db_data(const char *data, FIRST_INT_TYPE length, crc32_t *ex_crc); // returns offset
    //IGrowBuf64::size_type add_db_data(const char *data, size_t length, crc32_t *ex_crc) { assert(length <= 0x7FFFFFFF); return add_db_data(data, (int)length, ex_crc); }
    int add_data(const char *data, int length, IGrowBuf *dblock, crc32_t *ex_crc); // returns offset
    ...
    IGrowBuf64::size_type datablock_optimize(IGrowBuf64::size_type start_offset, FIRST_INT_TYPE first_int);
    bool datablock_finddata(IMMap&mmap, IGrowBuf64::size_type mmstart, FIRST_INT_TYPE size, IGrowBuf64::size_type*ofs);
#ifdef NSIS_CONFIG_CRC_SUPPORT
    bool datablock_crcdata(IMMap *in, IGrowBuf64::size_type start, FIRST_INT_TYPE len, crc32_t *ex_crc);
    bool datablock_crcdata(IGrowBuf *in, IGrowBuf::size_type start, FIRST_INT_TYPE len, crc32_t *ex_crc);
    ...
    /** Are we defining a stub installer version?
     * @param stub Use like a boolean to define whether in stub mode.
     */
    void set_datablock_mode(int stub);
#endif
    ...
    INT64 uninstall_size,uninstall_size_full;
    ...
    int build_external_file;
    unsigned int build_stub_installer_size;
    TCHAR build_output_external_filename[1024];
#endif
...
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    UINT64 db_full_size_s;
#endif
    INT64 db_comp_save, db_comp_save_u;
    ...
    TinyGrowBuf64 build_datablock_cache, ubuild_datablock_cache;
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    MMapBuf build_datablock_stub;
    TinyGrowBuf64 build_datablock_stub_cache;
#endif
    IGrowBuf64 *cur_datablock, *cur_datablock_cache;
    struct cached_db_size
    {
      FIRST_INT_TYPE first_int; // size | (compressed ? FIRST_INT_FLAG : 0)
      IGrowBuf64::size_type start_offset;
    };
