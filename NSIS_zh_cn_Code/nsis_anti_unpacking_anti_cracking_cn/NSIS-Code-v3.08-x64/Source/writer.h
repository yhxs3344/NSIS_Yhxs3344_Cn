...
public:
  ...
  virtual void write_growbuf64(const IGrowBuf64 *b);

  virtual void write_data(const void *data, const UINT32 size) = 0;
  //virtual void write_data(const void *data, const UINT64 size);
...
public:
  ...
  virtual void write_data(const void *data, const UINT32 size);
...
class growbuf64_writer_sink : public writer_sink {
public:
  typedef IGrowBuf64 sink_type;
  growbuf64_writer_sink(sink_type *buf, const writer_target_info&ti) : writer_sink(ti), m_buf(buf) {  }
