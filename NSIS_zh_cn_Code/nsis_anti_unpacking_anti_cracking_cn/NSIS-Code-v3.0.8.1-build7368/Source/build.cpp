...
int CEXEBuild::add_page(int type)
{
  ...
  {
    ERROR_MSG(_T("错误: 无法添加 license page, NSIS_CONFIG_LICENSEPAGE 未定义.\n"));
    return PS_ERROR;
  }
#endif
  ...
  {
    ERROR_MSG(_T("错误: 无法添加 components page, NSIS_CONFIG_COMPONENTPAGE 未定义.\n"));
    return PS_ERROR;
  }
#endif
  ...
  if (type == PAGE_COMPONENTS)
  {
    ERROR_MSG(_T("错误: 无法添加 uninstConfirm page, NSIS_CONFIG_UNINSTALL_SUPPORT 未定义.\n"));
    return PS_ERROR;
  }
#endif
...
int CEXEBuild::write_output(void)
{
  ...
  if (!changed_target && !build_unicode)
    warning(DW_GENERIC_DEPRECATED, _T("ANSI 已弃用目标"));
...
retry_output:
  ...
  INFO_MSG(_T("%d 个页面%") NPRIs _T(" (%d 字节), "),np,np==1?_T(""):_T("s"),np*sizeof(page));
#endif
#define IsRequiredSection(s) ( (!(s).name_ptr && ((s).flags & SF_SELECTED)) || (((s).flags & (SF_RO|SF_SELECTED)) == (SF_RO|SF_SELECTED)))
  ...
  INFO_MSG(_T("%d 条指令%") NPRIs _T(" (%d 字节), "),ne,ne==1?_T(""):_T("s"),ne*sizeof(entry));
  int ns=build_strlist.getnum();
  INFO_MSG(_T("%d 个字串%") NPRIs _T(" (%d 字节), "),ns,ns==1?_T(""):_T("s"),build_strlist.gettotalsize());
  ...
  if (ubuild_entries.getlen())
  {
    ...
    if (PAGE_COMPLETED != PAGE_INSTFILES && np) --np; // Special page not part of count
    INFO_MSG(_T("%d 个页面%") NPRIs _T(" (%d 字节), "),np,np==1?_T(""):_T("s"),ubuild_pages.getlen());
#endif
    {
    ...
    INFO_MSG(_T("%d 条指令%") NPRIs _T(" (%d 字节), "),ne,ne==1?_T(""):_T("s"),ubuild_entries.getlen());
    ns=ubuild_strlist.getnum();
    INFO_MSG(_T("%d 个字串%") NPRIs _T(" (%d 字节), "),ns,ns==1?_T(""):_T("s"),ubuild_strlist.gettotalsize());
...

  {
    ...
    else
    {
      ...
      FriendlySize cs(dbsize, GFSF_BYTESIFPOSSIBLE | (us.UInt()==(dbsizeu <= (UINT32)0xffffffff ? (UINT32)dbsizeu : 0) ? GFSF_HIDEBYTESCALE : 0)); // compressed installer size
      INFO_MSG(_T("安装数据:             %10u%") NPRIs _T(" / %u%") NPRIs _T("\n"),
        cs.UInt(),(!_tcsicmp(cs.Scale(),us.Scale())?_T(""):cs.Scale()),us.UInt(),us.Scale()); // "123 / 456 bytes" or "123 KiB / 456 MiB"
    }
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    if (!build_compress_whole)
    {
  ...
  if (uninstall_size>=0)
  {
    ...
    {
      FriendlySize cs(uninstall_size, GFSF_BYTESIFPOSSIBLE | (us.UInt()==(uninstall_size_full <= 0xffffffff ? (UINT32)uninstall_size_full : 0) ? GFSF_HIDEBYTESCALE : 0));
      INFO_MSG(_T("卸载代码 + 数:          %6u / %u 字节\n"),cs.UInt(),us.UInt());
    }
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    if (build_external_file == OFM_DATA)
...
int CEXEBuild::uninstall_generate()
{
  ...
  if (ubuild_entries.getlen() && uninstaller_writes_used)
  {
    ...
    if (start_offset)
    {
      ...
      if (!in_buf)
      {
        ERROR_MSG(_T("错误: 无法从输入中读取 %d个 字节\n"), in_len);
        return PS_ERROR;
      }
      MMapBuf udata_in;
