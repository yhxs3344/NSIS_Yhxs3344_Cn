/*
  ...
  在任何情况下，作者均不对因使用本软件而导致的任何损害承担责任.

  允许任何人将本软件用于任何目的（包括商业应用），并对其进行修改和重新分发，
  但须遵守以下限制:

  1. 不得歪曲本软件的来源；不得声称您编写了原始软件. 若在产品中使用本软件，建议在文档中致谢，但非强制.
  2. 修改后的源代码版本必须明确标记为修改版本，不得歪曲为原始软件.
  3. 不得删除或修改本声明中的任何部分.

  Unicode 支持 by Jim Park -- 08/18/2007
*/
...
int WINAPI _tWinMain(HINSTANCE hInst,HINSTANCE hOldInst,LPTSTR CmdLineParams,int ShowCmd) {
  ...
  // 我们可以与 .nsi 文件关联，当从 shell 启动时，会继承当前目录, 
  // 因此需要阻止 LoadLibrary 搜索当前目录，因为其中可能包含不可信的 DLL!
  FARPROC SDDA = GetSysProcAddr("KERNEL32", "SetDllDirectoryA"); // WinXP.SP1+
  if (SDDA) ((BOOL(WINAPI*)(LPCSTR))SDDA)(""); // 从默认 DLL 搜索顺序中移除当前目录

  // 尝试注册 SysLink 类
  enum { icc_link_class = 0x8000 }; // ComCtl32v6
  #if (!defined(_MSC_VER) && !defined(_WIN64)) || (defined(_MSC_VER) && (defined(_M_IX86) || defined(_M_IA64))) // x86 或 Itanium
  if (!InitCCEx(icc_link_class) && (sizeof(void*) > 4 || LOBYTE(GetVersion()) >= 5)) // 必须检查版本, 因为旧的 shell32 在相同序号处有一个不兼容的函数
  {
    ...
    if (GetClassInfo(NULL, _T("Link Window"), &wc))
    {
      ...
      RegisterClass(&wc); // 如果 SysLink 不可用，则对旧链接窗口类进行超类化
    }
  }
  #endif
...
static void ToolBarSizeChanged(HWND hDlg)
{
  ...
  SetWindowPos(hEd, 0, pt.x, top, RectW(r), oldh + (pt.y - top), SWP_NOZORDER|SWP_NOACTIVATE); // 更新 IDC_LOGWIN 的位置和大小
}
...
static BOOL CALLBACK DialogResize(HWND hWnd, LPARAM param)
{
  ...
  case IDC_LOGWIN:
    ...
    {
      r2.top = 246, r2.bottom = 22 + 190; // 来自 .rc 文件的对话框单位
...
INT_PTR CALLBACK DialogProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
  switch (msg) {
    case WM_INITDIALOG:
    {
      ...
      // 由 Darren Owen (DrO) 于 2003年9月29日修改
      // 添加了对富编辑控件鼠标和键盘事件的接收
      ...
      else if (SupportsW2000() && GetACP() == 932) // 根据旧版 Inno，在 WinXP 以下 Courier New 无法显示日文
      {
        ...
        fontname = _T("MS Gothic"); // 可处理此字体，更低版本不行
    ...
    case WM_MAKENSIS_PROCESSCOMPLETE:
    {
      ...
      if(g_sdata.compressor == COMPRESSOR_BEST) {
        if (g_sdata.retcode==0 && FileExists(g_sdata.output_exe)) {
          TCHAR temp_file_name[1024]; // BUGBUG: 硬编码缓冲区大小
      ...
      if (!g_sdata.retcode) {
        ...
        // 由 Darren Owen (DrO) 于 2003年10月1日添加
    ...
    case MakensisAPI::QUERYHOST: {
      if (MakensisAPI::QH_OUTPUTCHARSET == wParam) {
        const UINT reqcp = 1200; // 我们需要 UTF-16LE
    ...
    case WM_NOTIFY:
      switch (((NMHDR*)lParam)->code ) {
        ...
        // 由 Darren Owen (DrO) 于 2003年10月6日修改
        // 允许在 Windows 2000 以下系统检测右键菜单,
        // 并模拟原本会收到的 WM_CONTEXTMENU 消息
        // 注意：移除了 WM_CONTEXTMENU 处理，以避免在 Windows 2000 及更高系统上出现重复菜单
        case EN_MSGFILTER:
          ...
          if(WM_RBUTTONUP == lpnmMsg->msg || (WM_KEYUP == lpnmMsg->msg && lpnmMsg->wParam == VK_APPS))
          {
            ...
            // 由 Darren Owen (DrO) 于 2003年9月29日添加并修改
            // 如果按下应用程序键且鼠标不在窗口内，则把右键菜单位于窗口左上角
            // 从这里开始...
    ...
    case WM_TIMER:
    {
      ...
      switch(wParam) {
        case TID_CONFIGURECLOSEORABORT:
          SendMessage(hCtl = GetDlgItem(hwndDlg, IDCANCEL), WM_SETTEXT, 0, (LPARAM) (g_sdata.thread ? _T("&终止") : _T("&关闭")));
    ...
    case WM_COMMAND:
    {
      switch (LOWORD(wParam)) {
        case IDM_UI_SWITCHSECTION: //devblogs.microsoft.com/oldnewthing/20191022-00/?p=103016
          SetDialogFocus(hwndDlg, g_toolbar.hwnd); // 工具栏没有 WS_TABSTOP，也没有其他可切换的 "UI 区域" 因此直接跳转到那里
          break;
        case IDM_BROWSESCR: {
        ...
        case IDM_GUIDGEN:
        {
          ...
          for (UINT i = 0; sizeof(TCHAR) < 2; ++i) if (!(buf[i] = (CHAR) ((WCHAR*)buf)[i])) break; // 如果是 ANSI，将 WCHAR 转换为 TCHAR
          LogMessage(g_sdata.hwnd, (buf[38] = '\r', buf[39] = '\n', buf[40] = '\0', buf));
          SendMessage(g_sdata.hwnd, WM_MAKENSIS_UPDATEUISTATE, 0, 0); // 更新清除日志命令状态
          break;
        }
        case IDM_ARP:
        {
        ...
        case IDM_SAVE:
        {
          ...
          if (GetSaveFileName(&l)) {
            ...
            if (INVALID_HANDLE_VALUE != hFile) { // BUGBUG:TODO: 针对 hFile 无效或 EM_STREAMOUT 失败显示错误消息?
...
DWORD WINAPI MakeNSISProc(LPVOID TreadParam) {
  ...
  if (!g_sdata.sigint_event) {
    ErrorMessage(g_sdata.hwnd, _T("在创建中止事件时出错."));
  ...
  if (!CreateProcess(0, g_sdata.compile_command, 0, 0, TRUE, CREATE_NEW_CONSOLE, 0, 0, &si, &pi)) {
    TCHAR buf[MAX_STRING]; // BUGBUG: TODO: 太小了吗?
  ...
  CloseHandle(newstdout); // 立即关闭此句柄 (子进程中重复的) 以便获得 ERROR_BROKEN_PIPE
  ...
  for(;;)
  {
    ...
    if (!cch)
    {
      if (!rok) break; // TODO: 如果 cb 非零 应报告不完整读取错误?
      cbofs += cbio; // 我们只有 1 个字节, 需要读取更多以获得完整的 WCHAR
      continue;
    }
    char oddbyte = (char)(cb % 2), incompsurr;
    cbofs = 0;
    if ((incompsurr = IS_HIGH_SURROGATE(p[cch-1])))
      wcl = p[--cch], cbofs = sizeof(WCHAR); // 存储高代理项部分,稍后完成
    if (oddbyte)
...
static INT_PTR CALLBACK AboutProc(HWND hwndDlg, UINT msg, WPARAM wParam, LPARAM lParam) {
  ...
  switch(msg) {
    case WM_DRAWITEM:
      if (wParam == IDC_ABOUTHEADER)
      {
        ...
        if (!dd.FinalHeaderPos)
        {
          ...
          dd.FinalHeaderPos -= DlgUnitToPixelX(hwndDlg, 7); // 边距
        }
        r.left += dd.AnimPos;
    ...
    case WM_INITDIALOG:
    {
      ...
      if (!fontbold && (SupportsWNT4() || SupportsW95())) { // Tahoma 随 98+ 和 2000+ 发布
      ...
      SendMessage(hwndDlg, WM_APP, 0, 0); // 设置 IDC_ABOUTVERSION
      ...
      switch (((NMHDR*)lParam)->code)
      {
        ...
        case NM_RETURN:
          if (((NMHDR*)lParam)->idFrom == IDC_ABOUTDONATE)
          {
            ...
            for (UINT i = 0;; ++i) if (!(url[i] = (char) (encurl[i] & ~x))) break; // "解密" URL
...
static void FixSimpleComboBoxSizeBug(HWND hCombo) // 修复 Win10 中 CBS_SIMPLE 在 DPI != 96 时的高度绘制错误
{
  RECT r;
...
INT_PTR ShowSymbolSetDialog(HWND hwndOwner, BOOL LoadingSet)
{
  ...
  g_ModalDlgData = dd.pOldMDD; // 恢复旧指针
  return retval;
}
