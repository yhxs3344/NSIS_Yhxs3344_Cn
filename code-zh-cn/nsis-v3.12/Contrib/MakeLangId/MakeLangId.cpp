...
=======
20161207 - anders_k
* 从头重写，支持 EnumSystemLocales 中的语言
+ 新图标（"Swerica"，由 anders_k 提供，采用 ZLib 与 CC BY-SA 4.0 双许可）
...
#if defined(_MSC_VER) && _MSC_VER-0 >= 1600 && _MSC_VER-0 <= 1800 // MSVC 抱怨我们根本没在用的数学内容 (bug #1159)
...
typedef struct { WORD id; LPCSTR name; } INTLNG; // 以 ASCII 存储名称，节省 4 KiB
...
static const INTLNG g_IntLang[] = {
	...
	IL(0x7f, 0x00, INVARIANT, NEUTRAL), // 固定区域设置
	...
	IL(0x29, 0x01, FARSI, PERSIAN_IRAN), // LANG_FARSI 即 LANG_PERSIAN
	...
	IL(0x1a, 0x02, SERBIAN, SERBIAN_LATIN), // 塞尔维亚和黑山（前）
	IL(0x1a, 0x03, SERBIAN, SERBIAN_CYRILLIC), // 塞尔维亚和黑山（前）
...
static BOOL CALLBACK EnumSysLocalesProc(LPTSTR lpLocaleString)
{
	...
	if (!retval) // LOCALE_SENGLISHDISPLAYNAME 是 Win7 引入的
	{
...
{
	g_hList = hCtl; // 很蠢，没有回调参数
	SendMessage(hCtl, CB_SETCUEBANNER, 0, (LPARAM) L"选择语言...");
...
static void OnLanguageChanged(HWND hDlg)
{
	...
	if (!retval) // LOCALE_SNATIVEDISPLAYNAME 是 Win7 引入的
	{
	...
	LPCTSTR cpstr = retval ? lstrcmp(buf2, TEXT("0")) ? buf2 : TEXT("1200 (仅 Unicode)") : TEXT("?");
...
INT_PTR CALLBACK DialogProc(HWND hDlg, UINT Msg, WPARAM wp, LPARAM lp)
{
	...
	{
	case WM_SIZE:
		if (wp == SIZE_MAXIMIZED) ShowWindow(hDlg, SW_SHOWNOACTIVATE); // 禁止 STARTF_USESHOWWINDOW+SW_MAXIMIZE
		break;
	...
	case WM_INITDIALOG:
		{
			...
			SendMessage(hCtl, CB_ADDSTRING, 0, (LPARAM) TEXT("系统语言"));
			SendMessage(hCtl, CB_ADDSTRING, 0, (LPARAM) TEXT("内置语言"));
			SendMessage(hCtl, CB_SETCURSEL, 0, 0); // 继续向下执行 "系统"...
			SendMessage(hDlg, WM_COMMAND, MAKELONG(IDC_SOURCE, CBN_SELCHANGE), (LPARAM) hCtl); // ...并通知
			PostMessage(hDlg, WM_NEXTDLGCTL, 0, FALSE); // 将焦点切换到第二个下拉框
		}
		break;
	}
	return FALSE;
