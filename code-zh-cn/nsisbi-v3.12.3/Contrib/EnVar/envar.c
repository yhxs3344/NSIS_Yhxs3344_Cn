 ...
 *
 * 2014-2016, 2018, 2020-2021, 2026 MouseHelmet Software.
 *
...
{
  int pos = 0, length = 0, len = 0;
  PTCHAR out;
  struct VARITEM *count;
  
  if (!p_list) return NULL;

  len = GetStrDataLen(p_list);
  out = StrAlloc(len+APPEND_SIZE);
  if (!out) return NULL;
...
/* Sets a value to a variable. Blocks the %path% from being overwritten. */
LPCTSTR SetVarValue(LPCTSTR ptcVarName, PTCHAR ptPathString, DWORD dwKey)
{
  DWORD dwRet;

  if (!lstrcmpi(ptcVarName, _T("path"))) return ERR_NOWRITE;

  dwRet = WriteRegVar(ptcVarName, dwKey, ptPathString, StrLen(ptPathString)+1);
  if (dwRet != ERROR_SUCCESS) return ERR_NOWRITE;
...
{
  struct VARLIST list;
  PTCHAR ptVarName = StrAlloc(string_size+APPEND_SIZE);
  PTCHAR ptPathString = StrAlloc(string_size+APPEND_SIZE);
...
/* This routine adds a REG_SZ value in a environment variable (checks for existing paths first). */
__declspec(dllexport) void SetValue(HWND hwndParent, int string_size, TCHAR *variables, stack_t **stacktop, extra_parameters* xp)
{
  PTCHAR ptVarName = StrAlloc(string_size+APPEND_SIZE);
  PTCHAR ptPathString = StrAlloc(string_size+APPEND_SIZE);
  ...
  pushstring(SetVarValue(ptVarName, ptPathString, REG_SZ));
...
/* This routine adds a REG_EXPAND_SZ value in a environment variable (checks for existing paths first). */
__declspec(dllexport) void SetValueEx(HWND hwndParent, int string_size, TCHAR *variables, stack_t **stacktop, extra_parameters* xp)
{
  PTCHAR ptVarName = StrAlloc(string_size+APPEND_SIZE);
  PTCHAR ptPathString = StrAlloc(string_size+APPEND_SIZE);
  ...
  pushstring(SetVarValue(ptVarName, ptPathString, REG_EXPAND_SZ));
...
{
  struct VARLIST list;
  PTCHAR ptRegRoot = StrAlloc(string_size+APPEND_SIZE);
  PTCHAR ptVarName = StrAlloc(string_size+APPEND_SIZE);
