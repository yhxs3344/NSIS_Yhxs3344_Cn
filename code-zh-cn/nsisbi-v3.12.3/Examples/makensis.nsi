...
InstallDir $PROGRAMFILES${BITS}\NSISBI
InstallDirRegKey HKLM Software\NSISBI ""
...
;Names
Name "NSISBI"
Caption "NSISBI ${VERSION}${NAMESUFFIX} Setup"

!define REG_UNINST_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\NSISBI"
...
;Pages
!define MUI_WELCOMEPAGE_TITLE "Welcome to the NSISBI ${VERSION} Setup Wizard"
!define MUI_WELCOMEPAGE_TEXT "This wizard will guide you through the installation of NSISBI (Nullsoft Scriptable Install System Big Install) ${VERSION}, the next generation of the Windows installer and uninstaller system that doesn't suck and isn't huge.$\r$\n$\r$\nNSISBI expands on NSIS and adds support for multithreaded compression and decompression, 4GB all-in-one installers, and stub mode installers bigger than 4GB with single or multiple extra files (does not support disk spanning, all files must be accessible at install time). NSISBI is designed as a drop-in replacement for NSIS.$\r$\n$\r$\n$_CLICK"
...
!define MUI_FINISHPAGE_LINK "Visit the NSISBI site for the latest news, FAQs and support"
!define MUI_FINISHPAGE_LINK_LOCATION "http://sf.net/projects/nsisbi"
...
VIAddVersionKey "FileDescription" "NSISBI Setup"
...
!macro InstallStub stub
  !if ${BITS} >= 64
    ...
    !insertmacro OptionalFile ..\Stubs\${stub}-x86-ansi '' 'x86 != ${NSIS_CPU}'
    !insertmacro OptionalFile ..\Stubs\${stub}-x86-unicode '' 'x86 != ${NSIS_CPU}'
  !else
...
${MementoSection} "NSISBI Core Files (required)" SecCore

  SetDetailsPrint textonly
  DetailPrint "Installing NSISBI Core Files..."
...
!endif
  ...
  !insertmacro InstallStub lz4
  !insertmacro InstallStub lz4_solid
...
${MementoSection} "Start Menu Shortcut" SecShortcuts
  ...
  SectionIn 1 2
  SetOutPath $INSTDIR
  CreateShortcut "$SMPROGRAMS\NSISBI${NAMESUFFIX}.lnk" "$INSTDIR\NSIS.exe"
...
${MementoSection} "EnVar" SecPluginsEnVar

  SetDetailsPrint textonly
  DetailPrint "Installing Plug-ins | EnVar..."
  SetDetailsPrint listonly

  SectionIn 1

  !insertmacro InstallPlugin EnVar
  SetOutPath $INSTDIR\Docs\EnVar
  File ..\Docs\EnVar\license.txt
  File ..\Docs\EnVar\readme.txt
  SetOutPath $INSTDIR\Examples\EnVar
  File ..\Examples\EnVar\example.nsi
${MementoSectionEnd}
...
Section -UninstallerAndAssoc SecUninstallerAndAssoc
  ...
  WriteRegStr HKLM "Software\NSISBI" "" $INSTDIR
!ifdef VER_MAJOR & VER_MINOR & VER_REVISION & VER_BUILD
  WriteRegDword HKLM "Software\NSISBI" "VersionMajor" "${VER_MAJOR}"
  WriteRegDword HKLM "Software\NSISBI" "VersionMinor" "${VER_MINOR}"
  WriteRegDword HKLM "Software\NSISBI" "VersionRevision" "${VER_REVISION}"
  WriteRegDword HKLM "Software\NSISBI" "VersionBuild" "${VER_BUILD}"
!endif
  ...
  WriteRegStr HKLM "${REG_UNINST_KEY}" "UninstallString" '"$INSTDIR\uninst-nsisbi.exe"'
  WriteRegStr HKLM "${REG_UNINST_KEY}" "QuietUninstallString" '"$INSTDIR\uninst-nsisbi.exe" /S'
  WriteRegStr HKLM "${REG_UNINST_KEY}" "InstallLocation" "$INSTDIR"
  WriteRegStr HKLM "${REG_UNINST_KEY}" "DisplayName" "Nullsoft Install System Big Install${NAMESUFFIX}"
...
!endif
  WriteRegStr HKLM "${REG_UNINST_KEY}" "Publisher" "Nullsoft, Contributors, and Jason Ross" ; Required by WACK
  WriteRegStr HKLM "${REG_UNINST_KEY}" "URLInfoAbout" "https://sf.net/projects/nsisbi/"
  WriteRegStr HKLM "${REG_UNINST_KEY}" "HelpLink" "https://sf.net/projects/nsisbi/"
  ...
  SetOutPath $INSTDIR
  WriteUninstaller $INSTDIR\uninst-nsisbi.exe
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
  !insertmacro MUI_DESCRIPTION_TEXT ${SecCore} "The core files required to use NSISBI (compiler etc.)"
  !insertmacro MUI_DESCRIPTION_TEXT ${SecExample} "Example installation scripts that show you how to use NSISBI"
  ...
  !insertmacro MUI_DESCRIPTION_TEXT ${SecInterfacesDefaultUI} "The default NSISBI user interface which you can customize to make your own UI"
  !insertmacro MUI_DESCRIPTION_TEXT ${SecInterfacesTinyUI} "A tiny version of the default user interface"
  !insertmacro MUI_DESCRIPTION_TEXT ${SecTools} "Tools that help you with NSISBI development"
  !insertmacro MUI_DESCRIPTION_TEXT ${SecToolsZ2E} "A utility that converts a ZIP file to a NSISBI installer"
  ...
  !insertmacro MUI_DESCRIPTION_TEXT ${SecPluginsPlugins} "Useful plugins that extend NSISBI's functionality"
  ...
  !insertmacro MUI_DESCRIPTION_TEXT ${SecPluginsnsExec} "Plugin that executes console programs and prints its output in the NSISBI log window or hides it"
  ...
  !insertmacro MUI_DESCRIPTION_TEXT ${SecPluginsEnVar} "Plugin that lets you add and remove environment paths"
...
Function PageReinstall
  ...
  ReadRegStr $R0 HKLM "Software\NSISBI" ""
  ReadRegStr $R1 HKLM "${REG_UNINST_KEY}" "UninstallString"
  ${IfThen} "$R0$R1" == "" ${|} Abort ${|}

  StrCpy $R4 "older"
  ReadRegDWORD $R0 HKLM "Software\NSISBI" "VersionMajor"
  ReadRegDWORD $R1 HKLM "Software\NSISBI" "VersionMinor"
  ReadRegDWORD $R2 HKLM "Software\NSISBI" "VersionRevision"
  ReadRegDWORD $R3 HKLM "Software\NSISBI" "VersionBuild"
  ...
  ${If} $R0 == 0
    StrCpy $R1 "NSISBI ${VERSION} is already installed. Select the operation you want to perform and click Next to continue."
    StrCpy $R2 "Add/Reinstall components"
    StrCpy $R3 "Uninstall NSISBI"
    !insertmacro MUI_HEADER_TEXT "Already Installed" "Choose the maintenance option to perform."
    StrCpy $R0 "2"
  ${ElseIf} $R0 == 1
    StrCpy $R1 "An $R4 version of NSISBI is installed on your system. It's recommended that you uninstall the current version before installing. Select the operation you want to perform and click Next to continue."
    StrCpy $R2 "Uninstall before installing"
    StrCpy $R3 "Do not uninstall"
    !insertmacro MUI_HEADER_TEXT "Already Installed" "Choose how you want to install NSISBI."
    StrCpy $R0 "1"
  ${ElseIf} $R0 == 2
    StrCpy $R1 "A newer version of NSISBI is already installed! It is not recommended that you install an older version. If you really want to install this older version, it's better to uninstall the current version first. Select the operation you want to perform and click Next to continue."
    StrCpy $R2 "Uninstall before installing"
    StrCpy $R3 "Do not uninstall"
    !insertmacro MUI_HEADER_TEXT "Already Installed" "Choose how you want to install NSISBI."
    StrCpy $R0 "1"
...
Section Uninstall
  ...
  IfFileExists $INSTDIR\Bin\makensis.exe nsis_installed
    MessageBox MB_YESNO "It does not appear that NSISBI is installed in the directory '$INSTDIR'.$\r$\nContinue anyway (not recommended)?" IDYES nsis_installed
  ...
  DeleteRegKey HKLM "Software\NSISBI"
  DeleteRegKey HKCU "Software\NSISBI\MRU"
  DeleteRegKey /IfNoSubkeys HKCU "Software\NSISBI"

  SetDetailsPrint textonly
  DetailPrint "Deleting Files..."
  SetDetailsPrint listonly

  Delete "$SMPROGRAMS\NSISBI${NAMESUFFIX}.lnk"
  Delete "$DESKTOP\NSISBI${NAMESUFFIX}.lnk" ; Remove legacy shortcut
  ...
  Delete $INSTDIR\uninst-nsisbi.exe
  Delete $INSTDIR\nsisconf.nsi
  Delete $INSTDIR\nsisconf.nsh
