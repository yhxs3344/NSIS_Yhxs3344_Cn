...
void LZMACALL lzmaEnd(lzma_stream *s)
{
  if (s->dictionary) lzmafree(s->dictionary), s->dictionary = NULL;
  if (s->dynamicData) lzmafree(s->dynamicData), s->dynamicData = NULL;
}
