...
bool IMMap::write_to_external_file(FILE*file, UINT64 size)
{
  ...
  while (left > 0)
  {
    ...
    void *view = getmore(written, chunksize);
...
#ifdef _WIN32
int MMapFile::internalsetfile(HANDLE hFile, UINT64 dwSize)
#else
int MMapFile::internalsetfile(int hFile, UINT64 dwSize)
#endif
{
  clear();
...
void MMapFile::resize(UINT64 newsize)
{
  release();
...
void *MMapFile::get(UINT64 offset, size_t size) const
{
  return get(offset, &size);
}

void *MMapFile::get(UINT64 offset, size_t *sizep) const
{
  if (!sizep)
    return NULL;
  ...
  // fix offset
  maxfilesizetype alignedoffset = offset - (offset % m_iAllocationGranularity);
  size += (size_t)(offset - alignedoffset);

#ifdef _WIN32
  DB_OFFSET off;
  off.quad_part = alignedoffset;
  m_pView = MapViewOfFile(m_hFileMap, m_bReadOnly ? FILE_MAP_READ : FILE_MAP_WRITE, off.high_part, off.low_part, size);
#else
...
void *MMapFile::getmore(UINT64 offset, size_t size) const
{
  void *pView;
  void *pViewBackup = m_pView;
#ifndef _WIN32
  maxfilesizetype iMappedSizeBackup = m_iMappedSize;
#endif
  m_pView = 0;
...
UINT64 MMapFake::getsize() const
{
  return m_iSize;
}

void *MMapFake::get(UINT64 offset, size_t size) const
{
  return get(offset, &size);
}
...
void *MMapFake::getmore(UINT64 offset, size_t size) const
{
  return get(offset, size);
}

void MMapFake::resize(UINT64 n) {}
...
void MMapBuf::setlimit(UINT64 limit)
{
  cur_limit = limit;
}

UINT64 MMapBuf::getlimit()
{
  return cur_limit;
}

UINT64 MMapBuf::add(const void *data, int len) 
{ 
  if (len <= 0) return 0;
  resize(getlen() + len);
...
void MMapBuf::resize(UINT64 newlen)
{
  ...
  if (newlen > m_alloc)
  {
    if (!ull_add(m_alloc, newlen, (16 << 20))) // add 16mb to top of mapping
    ...
    if (m_gb.getlen())
    {
      // These typecasts should be fine, we aren't going over 16mb anyway
      memcpy(m_fm.get(0, (size_t)m_gb.getlen()), m_gb.get(), (size_t)m_gb.getlen());
...
UINT64 MMapBuf::getsize() const
{
  return getlen();
}

UINT64 MMapBuf::getlen() const
{
  if (m_gb_u)
    return m_used;
  return m_gb.getlen();
...
{
  // Typecast only applies to growbuf based blocks
  return get(0, (size_t)m_alloc);
}

void *MMapBuf::get(UINT64 offset, size_t *sizep) const
{
  if (!sizep)
    return NULL;
  size_t size = *sizep;
...
void *MMapBuf::get(UINT64 offset, size_t size) const
{
  if (m_gb_u)
...
void *MMapBuf::getmore(UINT64 offset, size_t size) const
{
  if (m_gb_u)
