...
unsigned int get_logical_cpu_count()
{
  ...
  count = sysctlbyname("hw.ncpu", &ct, &size, NULL, 0) ? 0 : ct;
#elif defined(_GNU_SOURCE)
  count = get_nprocs();
#elif defined(PTW32_VERSION) || defined(__hpux)
  count = pthread_num_processors_np();
#else
  // Even 1 compression thread can make things
  // roughly 40% faster than single main thread only
  count = 1;
#endif
...
const TCHAR* GetFriendlySize(UINT64 n, unsigned int&fn, GETFRIENDLYSIZEFLAGS f)
{
  static const TCHAR* scale[] = {
    _T(" bytes"), _T(" KiB"), _T(" MiB"), _T(" GiB"), _T(" TiB"), _T(" PiB"), _T(" EiB")
  };
