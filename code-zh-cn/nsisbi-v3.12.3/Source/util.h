...
static inline bool ull_add(UINT64 &r, UINT64 a, UINT64 b) { return (r = a + b) >= a; }
static inline bool sll_add(INT64 &r, INT64 a, INT64 b)
{
  UINT64 na = a < 0, aa = na ? a * -1 : a, nb = b < 0, ab = nb ? b * -1 : b, t = aa + ab, uz = 0;
  return na == nb && (t > LLONG_MAX || (~uz / 2 > LLONG_MAX && t < aa)) ? false : (r = a + b, true); // Good enough for our use
}
