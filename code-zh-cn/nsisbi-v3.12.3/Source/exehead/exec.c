...
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
extern LARGE_INTEGER g_ex_length;
#endif
...
#define parm6 (lent.offsets[6])
#define parm7 (lent.offsets[7])
...
#pragma GCC diagnostic pop
  ...
  switch (which)
  {
    ...
    case EW_EXTRACTFILE:
      ...
      _tryagain:
        if (overwriteflag >= 3) // check date and time
        {
          ...
          if (ffd)
          {
            cmp=CompareFileTime(&ffd->ftLastWriteTime, (FILETIME*)(lent.offsets + 4));
          }
        ...
        if (hOut == INVALID_HANDLE_VALUE)
        {
          ...
          GetNSISString(buf1,parm6); // use $0
        ...
        update_status_text(LANG_EXTRACT,buf3);
        {
          ...
          offset.LowPart = parm2;
          offset.HighPart = parm3;
          g_exec_flags.status_update++;
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
          if (which == EW_EXTRACTSTUBFILE)
            ret=GetCompressedDataFromExeHead(offset.QuadPart,hOut,parm7);
          else
#endif
          {
            ret=GetCompressedDataFromDataBlock(offset.QuadPart,hOut,parm7);
          }
          g_exec_flags.status_update--;
        }

        log_printf3(_T("File: wrote %I64d to \"%s\""),ret,buf0);

        if (parm4 != 0xffffffff || parm5 != 0xffffffff)
          SetFileTime(hOut,(FILETIME*)(lent.offsets+4),NULL,(FILETIME*)(lent.offsets+4));
    ...
    break;
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    case EW_VERIFYEXTERNALFILE:
      {
        exec_error++;
        if (g_dbex_hasFile && g_is_stub_installer)
          exec_error--;
        else if (g_is_stub_installer)
        {
          TCHAR *buf0 = GetStringFromParm(0x00);
          
          if (!validpathspec(buf0))
          {
            mystrcpy(buf1, state_exe_path);
            trimslashtoend(buf1);
            wsprintf(buf0, _T("%s\\setup1.bin"), buf1);
          }
          validate_filename(buf0);
          trimslashtoend(buf0);
          g_dbex_hasFile = OpenExternalFile(buf0);
          if (g_dbex_hasFile)
            exec_error--;
        }
      }
    ...
    case EW_WRITEREG: // write registry value
      {
        HKEY hKey;
        int rootkey=parm0, type=parm5, rtype=parm6;
        TCHAR *buf0=GetStringFromParm(0x02);
        TCHAR *buf1=GetStringFromParm(0x11);
        const TCHAR *rkn UNUSED=RegKeyHandleToName((HKEY)rootkey);
...
#endif
            LARGE_INTEGER offset;
            offset.LowPart = parm3;
            offset.HighPart = parm4;
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
            // use buf2, buf3 and buf4
            if (g_is_stub_installer)
              size = (int)GetCompressedDataFromExeHeadToMemory(offset.QuadPart, data, (3 * NSIS_MAX_STRLEN)*sizeof(TCHAR), parm7);
            else
#endif
            {
              size = (int)GetCompressedDataFromDataBlockToMemory(offset.QuadPart, data, (3 * NSIS_MAX_STRLEN)*sizeof(TCHAR), parm7);
            }
    ...
    case EW_WRITEUNINSTALLER:
      {
        FIRST_INT_TYPE ret=-666;
        HANDLE hFile;
        TCHAR *buf1=GetStringFromParm(-0x10);

        if (!validpathspec(buf1))
          GetStringFromParm(-0x14);

        remove_ro_attr(buf1);
        hFile=myOpenFile(buf1,GENERIC_WRITE,CREATE_ALWAYS);
        if (hFile != INVALID_HANDLE_VALUE)
        {
          LARGE_INTEGER dboffset;
          dboffset.LowPart = parm1;
          dboffset.HighPart = parm2;
          if (parm3)
          {
            ...
            if (filebuf)
            {
              ...
              {
                // parm1 = low_uninstdata_offset
                // parm2 = high_uninstdata_offset
                // parm3 = m_unicon_size
                // parm5 = icon crc
                // parm6 = data crc
                unsigned char* seeker;
                unsigned char* unicon_data = seeker = (unsigned char*)GlobalAlloc(GPTR,parm3);
                if (unicon_data) {
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
                  if (g_is_stub_installer)
                    GetCompressedDataFromExeHeadToMemory(dboffset.QuadPart,unicon_data,parm3,parm5);
                  else
#endif
                  {
                    GetCompressedDataFromDataBlockToMemory(dboffset.QuadPart,unicon_data,parm3,parm5);
                  }
                  while (*seeker) {
                    struct icondata {
                      DWORD dwSize;
...
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
          if (g_is_stub_installer)
            ret=GetCompressedDataFromExeHead(dboffset.QuadPart,hFile,parm6);
          else
#endif
          {
            ret=GetCompressedDataFromDataBlock(dboffset.QuadPart,hFile,parm6);
          }
          CloseHandle(hFile);
        }
        log_printf3(_T("created uninstaller: %I64d, \"%s\""),ret,buf1);
        {
