...
#include "../zlib/ZLIB.H"
#include "../mtw/mt_zlib_decompress.h"
#define z_stream mtzlib_decstream
#define inflateInitMT(x) dmtzlibInit((x), get_logical_cpu_count())
#define inflateResetMT(x) dmtzlibInit((x), get_logical_cpu_count())
#define inflateMT(x) dmtzlibDecode(x)
#define inflateEndMT(x) dmtzlibEnd(x)
#endif

#ifdef NSIS_COMPRESS_USE_LZ4
#include "../lz4/lz4decompress.h"
#include "../mtw/mt_lz4_decompress.h"
#define z_stream mtlz4_decstream
#define inflateInitMT(x) dmtlz4Init((x), get_logical_cpu_count())
#define inflateResetMT(x) dmtlz4Init((x), get_logical_cpu_count())
#define inflateMT(x) dmtlz4Decode(x)
#define inflateEndMT(x) dmtlz4End(x)
#define Z_OK LZ4_OK
...
#include "../mtw/mt_lzma_decompress.h"
#define z_stream mtlzma_decstream
#define inflateInitMT(x) dmtlzmaInit((x), get_logical_cpu_count())
#define inflateResetMT(x) dmtlzmaInit((x), get_logical_cpu_count())
#define inflateMT(x) dmtlzmaDecode(x)
#define inflateEndMT(x) dmtlzmaEnd(x)
#define Z_OK LZMA_OK
...
#include "../mtw/mt_bzip2_decompress.h"
#define z_stream mtbzip2_decstream
#define inflateInitMT(x) dmtbzip2Init((x), get_logical_cpu_count())
#define inflateResetMT(x) dmtbzip2Init((x), get_logical_cpu_count())
#define inflateMT(x) dmtbzip2Decode(x)
#define inflateEndMT(x) dmtbzip2End(x)
#define Z_OK BZ_OK
...
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
BOOL g_dbex_hasFile = FALSE;
#endif //NSIS_CONFIG_EXTERNAL_FILE_SUPPORT

#if defined(NSIS_CONFIG_COMPRESSION_SUPPORT) && defined(NSIS_COMPRESS_WHOLE)
HANDLE dbd_hFile=INVALID_HANDLE_VALUE;
static INT64 dbd_size, dbd_pos;
static UINT32 dbd_srcpos, dbd_fulllen;
static int NSISCALL __ensuredata(INT64 amount);
#endif//NSIS_COMPRESS_WHOLE

static UINT64 m_length;
static UINT64 m_pos;

static int calc_percent()
{
  UINT32 this_pos, this_len;
  ULARGE_INTEGER file_pos, file_len;

  file_len.QuadPart = m_length;
  file_pos.QuadPart = m_pos;

  if (file_len.HighPart)
  { 
    // 16MiB chunks, which gives a 64PiB display limit. Should be big enough for now :).
  ...
  return MulDiv(min(this_pos>>1,this_len>>1),100,this_len>>1);
}
...
retry:
  ...
  m_length = left = GetFileSize(db_hFile,NULL);
  while (left > 0)
  {
  ...
  if (g_has_external_file && !g_is_uninstaller)
  {
    ...
    trimslashtoend(path_ext);
    SetExternalFileSpecs(h.datablock_lowpart, h.datablock_highpart);
    g_dbex_hasFile = OpenExternalFile(path_ext);
    if (!g_dbex_hasFile && !g_is_stub_installer)
      return _LANG_INVALIDCRC;
  }
#endif //NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
...
#ifdef NSIS_COMPRESS_WHOLE
  inflateResetMT(&g_inflate_stream);
...
#ifdef NSIS_COMPRESS_WHOLE
  header->blocks[NB_DATA].offset = (UINT_PTR)dbd_pos;
#else
...
#ifdef NSIS_COMPRESS_WHOLE
  {
    // Since we are multithreaded now, might as well decompress everything up front
    LARGE_INTEGER off;
    off.LowPart = h.datablock_lowpart;
    off.HighPart = h.datablock_highpart;
    dbd_pos = 0;
    SetFilePointer64(dbd_hFile,dbd_pos,FILE_BEGIN);
    __ensuredata(off.QuadPart);
  }
#endif
...
{
  static unsigned char inbuffer[IBUFSIZE+OBUFSIZE];
  ...
  if (input_len & FIRST_INT_FLAG) // compressed
  {
    ...
    while (input_len > 0)
    {
...
#ifdef NSIS_CONFIG_CRC_SUPPORT
      if (g_do_crc) crc = CRC32(crc, inbuffer, l);
#endif
      ...
      for (;;)
      {
        ...
        err=inflateMT(&g_inflate_stream);

        if (err<0) return -4;

        u=BUGBUG64TRUNCATE(int, g_inflate_stream.next_out - outbuffer);

        tc=GetTickCount();
        if (g_exec_flags.status_update & 1 && (tc - ltc > 200 || !input_len))
        {
          m_length = input_len_total;
          m_pos = input_len_total - input_len;
          wsprintf(progress, _T("... %d%%"), calc_percent());
...
static unsigned char _inbuffer[IBUFSIZE];
static unsigned char _outbuffer[OBUFSIZE];
extern UINT64 m_length;
extern UINT64 m_pos;
  ...
  if (needed>0)
  {
    ...
    for (;;)
    {
      ...
      do
      {
        ...
        r=BUGBUG64TRUNCATE(DWORD,g_inflate_stream.next_out-_outbuffer);
        if (r)
        {
...
#endif
    dbd_pos=(INT64)g_blocks[NB_DATA].offset+offset;
    SetFilePointer64(dbd_hFile,dbd_pos,FILE_BEGIN);
  }
  retval=__ensuredata(FIRST_INT_SIZEOF);
  if (retval<0) return retval;

  if (!myReadFile(dbd_hFile,(LPVOID)&input_len_orig,FIRST_INT_SIZEOF)) return -3;
  input_len = input_len_orig;
...
#ifdef NSIS_CONFIG_CRC_SUPPORT
      if (g_do_crc) crc = CRC32(crc, _inbuffer, r);
#endif
