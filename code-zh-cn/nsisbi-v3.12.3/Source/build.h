...
class CEXEBuild {
  ...
  // Added by Sunil Kamath 11 June 2003
    ...
    int do_add_file(const TCHAR *lgss, int attrib, int recurse, int *total_files, const TCHAR
      *name_override=0, int generatecode=1, DB_OFFSET *data_handle=0, crc32_t *pcrc=0,
    ...
    int add_file(const tstring& dir, const tstring& file, int attrib, const TCHAR *name_override, int
      generatecode, DB_OFFSET *data_handle, crc32_t *pcrc, const TCHAR *which_token_str);
    ...
    IMMap::maxfilesizetype getcurdbsize();
    IMMap::maxfilesizetype getcurdblimit();
    ...
    DB_OFFSET add_db_data(IMMap *map, crc32_t *ex_crc); // returns offset
    DB_OFFSET add_db_data(const char *data, FIRST_INT_TYPE length, crc32_t *ex_crc); // returns offset
    //DB_OFFSET add_db_data(const char *data, size_t length, crc32_t *ex_crc) { assert(length <= 0x7FFFFFFF); return add_db_data(data, (int)length, ex_crc); }
    ...
    IMMap::maxfilesizetype datablock_optimize(IMMap::maxfilesizetype start_offset, FIRST_INT_TYPE first_int);
    bool datablock_finddata(IMMap&mmap, IMMap::maxfilesizetype mmstart, FIRST_INT_TYPE size, IMMap::maxfilesizetype*ofs);
    ...
    CZlibMT zlibmt_compressor;
    CBzip2MT bzip2mt_compressor;
    CLZMAMT lzmamt_compressor;
    CLZ4MT lz4mt_compressor;
#endif
    ...
    int build_compress_num_threads;
    ...
    UINT64 build_external_file_size;
    ...
    TinyGrowBuf build_datablock_stub_cache;
#endif
    MMapBuf *cur_datablock;
    IGrowBuf *cur_datablock_cache;
    struct cached_db_size
    {
      FIRST_INT_TYPE first_int; // size | (compressed ? FIRST_INT_FLAG : 0)
      IMMap::maxfilesizetype start_offset;
    };
