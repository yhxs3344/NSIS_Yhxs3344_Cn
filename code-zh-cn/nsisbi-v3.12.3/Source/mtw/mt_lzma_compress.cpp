...
unsigned int lzma_compress_thread(void *arg)
{
  PMTWJOB job = (PMTWJOB)arg;
  ICompressor *compressor = (CLZMA*)job->optional;
...
int CLZMAMT::add_threaded_job(const char *in, unsigned int size)
{
  ICompressor *compressor = new CLZMA;

  if (!compressor) return 0;

  PMTWJOB job = MakeJob(in, size, MT_LZMA_BLOCK_BUF_SIZE, in_seq_number, (void*)compressor);
  if (!job) return 0;

  job->comp_info.compressor = compressor_info.compressor;
  job->comp_info.dict_size = MT_LZMA_BLOCK_BUF_SIZE;  // Slight speedup if the dictionary is slightly bigger than the block size
  ...
  if (size < MT_LZMA_BLOCK_DATA_SIZE)
    lzma_compress_thread((void*)job);
  else
  {
    if (!c_ctx)
    {
    ...
    mt_add(c_ctx, (data_function)lzma_compress_thread, (void*)job);
  }
  return 1;
}
...
int CLZMAMT::Init(int level, unsigned int dict_size, unsigned int num_threads)
{
  int error = 0;
  if (init) End();
  ...
  if (!out_buf)
  {
    out_buf = (char *)_ALLOC(MT_LZMA_BLOCK_BUF_SIZE);
    error |= !out_buf;
  }
  else
  {
    for (int i = 0; i < MT_LZMA_BLOCK_BUF_SIZE; ++i)
      out_buf[i] = 0;
  }
  error |= !InitJobContext();

  init = !error;
  return error ? LZMA_MEM_ERROR : C_OK;
}
...
int CLZMAMT::Compress(bool finish)
{
  if (finished) return C_FINISHED;

  while (!(in_finish && out_finish))
  {
    if (out_seeker)
    {
      unsigned int shift = 0, header_bytes = out_seeker == out_buf ? MT_LZMA_BLOCK_HEADER_SIZE : 0;
      ...
      if (header_bytes)
      {
        ...
        for (i = 0; i < MT_LZMA_BLOCK_HEADER_SIZE; i++, shift += 8)
    ...
    else
    {
      ...
      if (JobHaveNextOut(out_seq_number) && !out_finish)
      {
        ...
        {
          out_seeker = out_end = NULL;
          return (ret_value < 0) ? LZMA_IO_ERROR : LZMA_MEM_ERROR;
        }
        if (out_end - out_buf)
          out_seeker = out_buf;
      }
      else
      {
        ...
        if (JobCountNextOut() < compressor_info.num_threads && !in_finish)
        {
          ...
          {
            unsigned int inputBytes = avail_in < MT_LZMA_BLOCK_DATA_SIZE ? avail_in : MT_LZMA_BLOCK_DATA_SIZE;
  ...
  if (in_finish && out_finish && finish)
  {
    int i;
    finished = true;
    if (avail_out < MT_LZMA_BLOCK_HEADER_SIZE) return LZMA_IO_ERROR;
    for (i = 0; i < MT_LZMA_BLOCK_HEADER_SIZE; ++i)
...
void CLZMAMT::SetNextIn(char *in, unsigned int size)
{
  next_in = in;
  avail_in = size;
}

void CLZMAMT::SetNextOut(char *out, unsigned int size)
{
  next_out = out;
  avail_out = size;
}

char* CLZMAMT::GetNextOut()
{
  return next_out;
}

unsigned int CLZMAMT::GetAvailIn()
{
  return avail_in;
}
unsigned int CLZMAMT::GetAvailOut()
{
  return avail_out;
}

const TCHAR* CLZMAMT::GetName()
{
  return _T("lzma");
}

const TCHAR* CLZMAMT::GetErrStr(int err)
{
  switch (err)
  {
  case LZMA_IO_ERROR:
