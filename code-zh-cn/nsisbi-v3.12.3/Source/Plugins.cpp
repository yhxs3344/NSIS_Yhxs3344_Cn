...
DB_OFFSET Plugins::GetDllDataHandle(bool uninst, const tstring& command) const
{
  const tstring dllname = GetDllName(command);
  DB_OFFSET error;
  error.quad_part = -1;
  if (uninst)
    return get_paired_value(m_dllname_to_unst_datahandle, dllname, error);
  else
    return get_paired_value(m_dllname_to_inst_datahandle, dllname, error);
}

crc32_t Plugins::GetDllCRCValue(bool uninst, const tstring& command) const
{
  const tstring dllname = GetDllName(command);
  if (uninst)
    return get_paired_value(m_dllname_to_unst_crc, dllname, -1);
  else
    return get_paired_value(m_dllname_to_inst_crc, dllname, -1);
}

void Plugins::SetDllDataHandle(bool uninst, tstring&canoniccmd, DB_OFFSET dataHandle, crc32_t crc)
{
  const tstring dllname = GetDllName(canoniccmd);
  if (uninst)
    m_dllname_to_unst_datahandle[dllname] = dataHandle,
    m_dllname_to_unst_crc[dllname] = crc;
  else
    m_dllname_to_inst_datahandle[dllname] = dataHandle,
    m_dllname_to_inst_crc[dllname] = crc;
}

bool Plugins::DllHasDataHandle(const tstring& dllnamelowercase) const
{
  DB_OFFSET h = GetDllDataHandle(false, dllnamelowercase);
  if (-1 == h.quad_part) h = GetDllDataHandle(true, dllnamelowercase);
  return -1 != h.quad_part;
}
