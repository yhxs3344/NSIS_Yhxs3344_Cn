...
LANGID CEXEBuild::ParseLangIdParameter(const LineParser&line, int token)
{
  ...
  if (!succ)
    warning_fl(DW_BAD_LANGID,_T("\"%") NPRIs _T("\" 错误的语言id,使用语言id %u!"),line.gettoken_str(token),lid);
  return lid;
}
...
int CEXEBuild::process_script(NIStream&Strm, const TCHAR *filename)
{
  ...
  {
    ERROR_MSG(_T("错误 (处理脚本): 已调用 write_output, 无法继续\n"));
    return PS_ERROR;
  }
  ...
  {
    ERROR_MSG(_T("错误: 脚本错误: 最后一行以 \\\n 结尾"));
    return PS_ERROR;
  }

  if (ret == PS_EOF && num_ifblock())
  {
    ERROR_MSG(_T("!if[macro][n]def: 在 EOF 处未关闭 - 需要 !endif\n"));
    return PS_ERROR;
  }
...
int CEXEBuild::doParse(const TCHAR *str)
{
  ...
  if (m_linebuild.getlen())
  {
    ...
    {
      warning_fl(DW_COMMENT_NEWLINE,_T("注释包含换行符,下一行将忽略"));
    }
  }
  ...
  if (res)
  {
    if (res==-2) ERROR_MSG(_T("错误: 解析行在 %") NPRIs _T(":%d\n 处存在未结束的字符串"),curfilename,linecnt);
    else ERROR_MSG(_T("错误: 解析行在 (%") NPRIs _T(":%d)\n 处出错"),curfilename,linecnt);
    return PS_ERROR;
  }
...
parse_again:
  ...
  if (tkid == -1)
  {
    ...
    if (p[0] && p[_tcslen(p)-1]==_T(':'))
    {
      ...
      {
        ERROR_MSG(_T("标签错误: %") NPRIs _T(" (标签不能以 !,$,-,+,或 0-9 开头)\n"),tokstr0);
        return PS_ERROR;
      }
...
#endif
    {
      ...
      if (Plugins::IsPluginCallSyntax(tokstr0))
      {
        ...
        ERROR_MSG(_T("插件%") NPRIs _T(" 未找到, 无法调用 %") NPRIs _T("\n"),m_pPlugins && m_pPlugins->IsKnownPlugin(tokstr0) ? _T(" function") : _T(""),tokstr0);
      }
      else
#endif
        ERROR_MSG(_T("命令无效: \"%") NPRIs _T("\"\n"),tokstr0);
      return PS_ERROR;
    }
  }
  ...
  {
    ERROR_MSG(_T("%") NPRIs _T(" 需要 %d"),tokstr0,np);
    if (op < 0) ERROR_MSG(_T("+"));
    if (op > 0) ERROR_MSG(_T("-%d"),op+np);
    ERROR_MSG(_T(" 个参数, 实际得到 %d.\n"),line.getnumtokens()-1);
    PRINTHELP()
  }
  ...
  if (tkid == TOK_P_ELSE)
  {
    ...
    if (!num_ifblock())
    {
      ERROR_MSG(_T("!else: 没有以if 开头 (!if[macro][n][def])\n"));
      return PS_ERROR;
    }
    
    if (cur_ifblock->elseused)
    {
      ERROR_MSG(_T("!else: 当前 if 中已使用 else\n"));
      return PS_ERROR;
    }
  ...
  if (tkid == TOK_P_ENDIF) {
    if (!num_ifblock())
    {
      ERROR_MSG(_T("!endif: 没有以if 开头 (!if[macro][n][def])\n"));
      return PS_ERROR;
    }
    end_ifblock();
    return PS_OK;
...
int CEXEBuild::LoadLicenseFile(const TCHAR *file, TCHAR** pdata, const TCHAR *cmdname, WORD AnsiCP) // caller must free *pdata, even on error result
{
  ...
  {
    ERROR_MSG(_T("%") NPRIs _T(": 打开失败 \"%") NPRIs _T("\"\n"),cmdname,file);
    print_help(cmdname);
    return PS_ERROR;
  }
  ...
  if (!cbFileData)
  {
    warning_fl(DW_LICENSE_EMPTY,_T("%") NPRIs _T(": 许可文件为空 \"%") NPRIs _T("\"\n"),cmdname,file);
  }
  else
  ...
  *pdata=data; // memory will be released by caller
  if (!data)
  {
    ERROR_MSG(_T("内部编译器错误 #12345: %") NPRIs _T(" 内存分配(%d) 失败.\n"),cmdname,cbTotalData);
    return PS_ERROR;
  }
  *((TCHAR*)((char*)data+cbTotalData-sizeof(TCHAR)))=_T('\0');
  ...
  {
l_errwcconv:
    ERROR_MSG(_T("%") NPRIs _T(": wchar_t 转换失败!\n"),cmdname);
    return PS_ERROR;
  }
...
int CEXEBuild::process_jump(LineParser &line, int wt, int *offs)
{
  ...
  else
  {
    ...
    {
      ERROR_MSG(_T("错误: 开头的 Goto 以 '+' 或 '-' 开头时后面必须跟随非零整数 (相对跳转)\n"));
      return 1;
    }
    if ((s[0] >= _T('0') && s[0] <= _T('9')) || s[0] == _T('$') || s[0] == _T('!'))
    {
      ERROR_MSG(_T("错误: Goto 跳转目标不能以 0-9,$,! 开头\n"));
      return 1;
    }
...
int CEXEBuild::doCommand(int which_token, LineParser &line)
{
  ...
  switch (which_token)
  {
    ...
    case TOK_P_MACROEND:
    return (ERROR_MSG(_T("!macroend: 当前没有打开的宏.\n")), PS_ERROR);
    ...
    case TOK_PAGE:
      {
        ...
        if (!k) {
          ERROR_MSG(_T("错误: 指定了自定义页面, 但未定义 NSIS_SUPPORT_CODECALLBACKS.\n"));
          return PS_ERROR;
        }
        ...
        if (k) { // not custom
          ...
          switch (line.getnumtokens() - enable_last_page_cancel) {
            ...
            case 3:
              ...
              break;
            case 2:
              ERROR_MSG(_T("错误: 自定义页面必须有一个创建函数!\n"));
              PRINTHELP();
          }
        }
#endif //~ NSIS_SUPPORT_CODECALLBACKS

        SCRIPT_MSG(_T("%") NPRIs _T("页面: %") NPRIs, uninstall_mode?_T("卸载"):_T(""), line.gettoken_str(1));
#ifdef NSIS_SUPPORT_CODECALLBACKS
    ...
    return PS_OK;
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_CODECALLBACKS.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_SUPPORT_CODECALLBACKS
#else
    case TOK_PAGE:
    ...
    case TOK_PAGEEXEND:
    case TOK_PAGECALLBACKS:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_VISIBLE_SUPPORT.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_LANGSTRING:
    {
      ...
      if (ret == PS_WARNING)
        warning_fl(DW_LANGSTRING_MULTISETWASTE,_T("LangString \"%") NPRIs _T("\" 对 %d 重复设置,浪费空间"),name,lang);
      else if (ret == PS_ERROR) {
        ERROR_MSG(_T("错误: 无法设置 LangString \"%") NPRIs _T("\"!\n"),name);
        return PS_ERROR;
      }
    ...
    case TOK_LANGSTRINGUP:
      SCRIPT_MSG(_T("错误: LangStringUP 已弃用, 不再存在未处理的字串. 请使用 LangString.\n"));
    return PS_ERROR;
    case TOK_LICENSELANGSTRING:
    {
      ...
      if (build_header.flags&(CH_FLAGS_SILENT|CH_FLAGS_SILENT_LOG))
        warning_fl(DW_LANGSTRING_SILENTLICENSE, _T("%") NPRIs _T(": 已启用 SilentInstall, 浪费空间"), cmdnam);
#endif
      TCHAR *name = line.gettoken_str(1);
      LANGID lang = ParseLangIdParameter(line, 2);
      ...
      if (ret == PS_WARNING)
        warning_fl(DW_LANGSTRING_MULTISETWASTE,_T("%") NPRIs _T(" \"%") NPRIs _T("\" 对 %d 多次设置,浪费空间"),cmdnam,name,lang);
      else if (ret == PS_ERROR)
      {
        ERROR_MSG(_T("错误: 无法设置 %") NPRIs _T(" \"%") NPRIs _T("\"!\n"), cmdnam, name);
        return PS_ERROR;
      }
    ...
    case TOK_NAME:
      {
        if (SetInnerString(NLF_NAME,line.gettoken_str(1)) == PS_WARNING)
          warning_fl(DW_INNERSTRING_MULTISETWASTE,_T("%") NPRIs _T(": 重复指定,浪费空间"),line.gettoken_str(0));
        tstring da;
        ...
        SCRIPT_MSG(_T("名称: \"%") NPRIs _T("\""),line.gettoken_str(1));
        if (*line.gettoken_str(2))
    ...
    case TOK_CHECKBITMAP:
      ...
      try {
        ...
        if (err) {
          switch (err) {
            case -1:
              ERROR_MSG(_T("错误: 未找到位图文件\n"));
              break;
            case -2:
              ERROR_MSG(_T("错误: 无效的位图文件 - 损坏或不是位图\n"));
              break;
            case -3:
              ERROR_MSG(_T("错误: 位图尺寸不是 96x16\n"));
              break;
            case -4:
              ERROR_MSG(_T("错误: 位图超过 8bpp\n"));
              break;
          }
          return PS_ERROR;
        }
        ...
        ERROR_MSG(_T("替换位图出错: %") NPRIs _T("\n"),CtoTStrParam(err.what()));
        return PS_ERROR;
      }
    return PS_OK;
#else
    case TOK_CHECKBITMAP:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_COMPONENTPAGE.\n"),  line.gettoken_str(0));
    return PS_ERROR;
...
#ifdef NSIS_CONFIG_VISIBLE_SUPPORT
      {
        if (!cur_page) {
          ...
          if (cur_page_type != PAGE_DIRECTORY) {
            ERROR_MSG(_T("错误: DirText 只能在 PageEx directory页面中使用.\n"));
            return PS_ERROR;
          }
    ...
    case TOK_DIRVAR:
    {
      if (cur_page_type != PAGE_DIRECTORY && cur_page_type != PAGE_UNINSTCONFIRM) {
        ERROR_MSG(_T("错误: 不能在 PageEx directory|uninstConfirm 页面以外使用 DirVar.\n"));
        return PS_ERROR;
      }
    ...
    {
      if (cur_page_type != PAGE_DIRECTORY) {
        ERROR_MSG(_T("错误: 不能在 PageEx directory 页面以外使用 DirVerify.\n"));
        return PS_ERROR;
      }
    ...
    case TOK_COMPTEXT:
      {
        if (!cur_page) {
          ...
          if (cur_page_type != PAGE_COMPONENTS) {
            ERROR_MSG(_T("错误: ComponentText 只能在 PageEx components 页面中使用.\n"));
            return PS_ERROR;
          }
          cur_page->parms[0] = add_string(line.gettoken_str(1));
          cur_page->parms[1] = add_string(line.gettoken_str(2));
    ...
    case TOK_INSTTYPE:
      {
        ...
        if (!_tcsicmp(line.gettoken_str(1),_T("/NOCUSTOM")))
        {
          ...
          SCRIPT_MSG(_T("InstType: 禁用自定义 %") NPRIs _T("安装类型\n"), _T(""));
        }
        ...
        else if (!_tcsicmp(line.gettoken_str(1),_T("/COMPONENTSONLYONCUSTOM")))
        {
          ...
          SCRIPT_MSG(_T("InstType: 使组件仅在自定义 %") NPRIs _T("安装类型时可见\n"), _T(""));
        }
        ...
        {
          SCRIPT_MSG(_T("InstType: 将自定义文本设置为: \"%") NPRIs _T("\"\n"),line.gettoken_str(1)+14);
          if (SetInnerString(NLF_COMP_CUSTOM,line.gettoken_str(1)+14) == PS_WARNING)
            warning_fl(DW_INNERSTRING_MULTISETWASTE,_T("%") NPRIs _T(": 重复指定,浪费空间"),_T("InstType /CUSTOMSTRING"));
        }
        ...
        else
        {
          ...
          {
            ERROR_MSG(_T("InstType: 最多允许 %d 种安装类型. 已指定 %d 种\n"), NSIS_MAX_INST_TYPES, NSIS_MAX_INST_TYPES + 1);
            return PS_ERROR;
          }
          else
          {
    ...
    case TOK_LICENSETEXT:
      {
        if (!cur_page) {
          ...
          if (cur_page_type != PAGE_LICENSE) {
            ERROR_MSG(_T("错误: LicenseText 只能在 PageEx license 页面中使用.\n"));
            return PS_ERROR;
          }
          cur_page->parms[0] = add_string(line.gettoken_str(1));
    ...
    case TOK_LICENSEDATA:
      {
        ...
        if (!cur_page) {
          if (SetInnerString(NLF_LICENSE_DATA,data) == PS_WARNING)
            warning_fl(DW_INNERSTRING_MULTISETWASTE,_T("%") NPRIs _T(": 重复指定,浪费空间"),cmdnam);
        }
        else {
          if (cur_page_type != PAGE_LICENSE) {
            ERROR_MSG(_T("错误: LicenseData 只能在 PageEx license 页面中使用.\n"));
            return PS_ERROR;
          }
    ...
    case TOK_LICENSEFORCESELECTION:
    {
      ...
      if (!cur_page) {
        ...
        if (cur_page_type != PAGE_LICENSE) {
          ERROR_MSG(_T("错误: LicenseForceSelection 只能在 PageEx license 页面中使用.\n"));
          return PS_ERROR;
        }
        switch (line.getnumtokens()) {
          case 4:
    ...
    case TOK_LICENSEDATA:
    case TOK_LICENSEBKCOLOR:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_LICENSEPAGE.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_SILENTINST:
    {
      ...
      if (k == 2)
      {
        ERROR_MSG(_T("SilentInstall: 指定了 silentlog, 但未编译日志支持 (请使用 NSIS_CONFIG_LOG) 启用记录功能\n"));
        return PS_ERROR;
      }
      ...
      {
        warning_fl(DW_LANGSTRING_SILENTLICENSE,_T("SilentInstall: LicenseData 重复指定,浪费空间"));
      }
      if (k) {
    ...
    return PS_OK;
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_UNINSTALL_SUPPORT.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif
    case TOK_IFSILENT:
    ...
    case TOK_IFSILENT:
    case TOK_SETSILENT:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_SILENT_SUPPORT.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    return PS_OK;
    case TOK_SETEXOUTFILE:
    case TOK_OUTFILEMODE:
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    {
      int arg;
      const TCHAR *str;
      if (which_token == TOK_SETEXOUTFILE)
      {
        arg = line.gettoken_enum(1,_T("auto\0off\0on\0stub\0")) - 1;
        warning_fl(DW_PARSE_SETEXOUTFILE_DEPRECATED, _T("SetExOutFile: is renamed to OutFileMode aio|stub"));
        arg = !!arg;
      }
      else
      {
        arg = line.gettoken_enum(1,_T("aio\0stub\0"));
      }
      switch (arg)
      {
        default:
      ...
      if (arg < OFM_AIO) PRINTHELP()
#ifdef NSIS_CONFIG_COMPRESSION_SUPPORT
      if (arg != OFM_AIO && build_compress_whole)
      {
        ERROR_MSG(_T("错误: %") NPRIs _T(" %") NPRIs _T(": 固体压缩不能与外部文件一起使用.\n"),line.gettoken_str(0),str);
        return PS_ERROR;
      }
      if (arg != build_external_file && build_compressor_set)
      {
        ERROR_MSG(_T("错误: %") NPRIs _T(" %") NPRIs _T(": 添加文件后无法更改输出文件模式.\n"),line.gettoken_str(0),str);
        return PS_ERROR;
      }
#endif
      build_external_file = arg;
      SCRIPT_MSG(_T("OutFileMode: %") NPRIs _T("\n"),str);
    }
    return PS_OK;
#else//!NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_EXTERNAL_FILE_SUPPORT.\n"),line.gettoken_str(0));
    return PS_ERROR;
#endif//NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    case TOK_OUTFILESIZE:
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    {
      int s;
      build_external_file_size = line.gettoken_int(1, &s);
      if (!s) PRINTHELP();
      SCRIPT_MSG(_T("OutFileSize: %llu MiB\n"), build_external_file_size);
      build_external_file_size <<= 20;
    }
    return PS_OK;
#else
      ERROR_MSG(_T("Error: %") NPRIs _T(" specified, NSIS_CONFIG_EXTERNAL_FILE_SUPPORT not defined.\n"), line.gettoken_str(0));
    return PS_ERROR;
#endif
    ...
    case TOK_INSTDIR:
    {
      ...
      {
        warning_fl(DW_STRING_MULTISETWASTE,_T("%") NPRIs _T(": 重复指定, 浪费空间"),line.gettoken_str(0));
      }
    ...
    case TOK_INSTALLDIRREGKEY: // InstallDirRegKey
      {
        ...
        if (line.gettoken_str(2)[0] == _T('\\'))
          warning_fl(DW_PARSE_REGPATHPREFIX, _T("%") NPRIs _T(": 注册表路径名称以 \'\\\', 开头时可能会出错"),line.gettoken_str(0));
    ...
    case TOK_INSTPROGRESSFLAGS:
      {
        ...
        try {
          ...
          ERROR_MSG(_T("设置平滑进度条时出错: %") NPRIs _T("\n"),CtoTStrParam(err.what()));
          return PS_ERROR;
        }
    ...
    case TOK_SHOWDETAILSUNINST:
      ...
      ERROR_MSG(_T("错误: 指定了 ShowUninstDetails 但未定义 NSIS_CONFIG_UNINSTALL_SUPPORT\n"));
      return PS_ERROR;
#endif
    ...
    case TOK_DIRSHOW:
     ...
     ERROR_MSG(_T("错误: DirShow 当前无法工作\n"));
    return PS_ERROR;
    ...
    case TOK_BGFONT:
      ...
      ERROR_MSG(_T("错误: 指定了 BGFont 但未定义 NSIS_SUPPORT_BGBG\n"));
      return PS_ERROR;
#else // NSIS_SUPPORT_BGBG
      if (line.getnumtokens()==1)
      {
        memcpy(&bg_font,&bg_default_font,sizeof(LOGFONT));
        SCRIPT_MSG(_T("BGFont: 默认字体\n"));
        return PS_OK;
      }
      LOGFONT newfont;
      newfont.lfHeight=40;
      ...
      SCRIPT_MSG(_T("BGFont: \"%") NPRIs _T("\""),line.gettoken_str(1));
      {
        ...
        for (int i = 2; i < line.getnumtokens(); i++) {
          ...
          if (tok[0]==_T('/')) {
            ...
            if (!height) {
              SCRIPT_MSG(_T(" 高度=%") NPRIs,tok);
              newfont.lfHeight=line.gettoken_int(i);
              height=true;
            }
            else if (!weight) {
              SCRIPT_MSG(_T(" 粗细=%") NPRIs,tok);
    ...
    case TOK_BGGRADIENT:
#ifndef NSIS_SUPPORT_BGBG
      ERROR_MSG(_T("错误: 指定了 BGGradient 但未定义 NSIS_SUPPORT_BGBG\n"));
      return PS_ERROR;
#else // NSIS_SUPPORT_BGBG
      if (line.getnumtokens()==1)
      {
        SCRIPT_MSG(_T("BGGradient: 默认颜色\n"));
    ...
    case TOK_INSTCOLORS:
    {
      ...
      if (p[0]==_T('/'))
      {
        ...
        SCRIPT_MSG(_T("InstallColors: windows 默认颜色\n"));
      }
      else
      {
    ...
    case TOK_CHANGEUI:
      try {
        ...
        #define CUISEARCHERR(n,v) ERROR_MSG(_T("错误: 未找到 %") NPRIs _T(" (%u) 在自定义 UI 中!\n"),n,v);
        ...
        ERROR_MSG(_T("更改 UI 时出错: %") NPRIs _T("\n"), CtoTStrParam(err.what()));
        return PS_ERROR;
      }
    return PS_OK;
    case TOK_ADDBRANDINGIMAGE:
      try {
        ...
        if (whtype != LU_DIALOG && !CDialogTemplate::SupportsDialogUnitComputation())
          throw runtime_error("必须在非 Win32 平台上使用对话框!");
        ...
        ERROR_MSG(_T("添加页眉图片出错: %") NPRIs _T("\n"),CtoTStrParam(err.what()));
        return PS_ERROR;
      }
    return PS_OK;
    case TOK_SETFONT:
    ...
    case TOK_TARGET:
    {
      ...
      {
        ERROR_MSG(_T("错误: 在设置 %") NPRIs _T(" (默认的 stub 时 出错)\n"),_T("architecture"));
        return PS_ERROR;
      }
    ...
    case TOK_TARGETUNICODE:
    {
      ...
      {
        ERROR_MSG(_T("错误: 数据已压缩 %") NPRIs _T(" 头部已更改后无法更改目标, 再更改压缩器!\n"), _T("字符集"));
        return PS_ERROR;
      }
...
#ifdef NSIS_CONFIG_COMPRESSION_SUPPORT
    {
      if (build_compressor_set) {
        ERROR_MSG(_T("错误: 数据已压缩或头部已更改后无法更改压缩器!\n"));
        return PS_ERROR;
      }
      if (build_compressor_final)
      {
        warning_fl(DW_COMP_FINAL, _T("由于之前使用了 /FINAL 开关, SetCompressor 被忽略"));
        return PS_OK;
      }
      ...
      while (line.gettoken_str(a)[0] == _T('/'))
      {
        ...
        else if (!_tcsicmp(line.gettoken_str(a),_T("/SOLID")))
        {
          ...
          if (build_external_file == OFM_STUB)
            warning_fl(DW_COMP_WHOLE_IGNORE, _T("SetCompressor: ignoring /SOLID flag due to OutFileMode stub (hint: call 'OutFileMode aio' before calling SetCompressor to use solid compression)"));
          else
#endif
          {
      ...
      {
        ERROR_MSG(_T("%") NPRIs _T(" 需要 %d 个参数,但得到 %d 个.\n"),line.gettoken_str(0),a + 1,line.getnumtokens());
        PRINTHELP();
      }

      int k=line.gettoken_enum(a, _T("zlib\0bzip2\0lzma\0lz4\0"));
      switch (k) {
        case 0: compressor = &zlibmt_compressor; break;
        case 1: compressor = &bzip2mt_compressor; break;
        case 2: compressor = &lzmamt_compressor; break;
        case 3: compressor = &lz4mt_compressor; break;
        default: PRINTHELP();
      }
      ...
      {
        SCRIPT_MSG(_T("压缩载入 stub 时 \"%") NPRIs _T("\" 出错.\n"),compressor_name.c_str());
        return PS_ERROR;
      }
    ...
    return PS_OK;
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T("但未定义 NSIS_CONFIG_COMPRESSION_SUPPORT.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_CONFIG_COMPRESSION_SUPPORT
    case TOK_LOADNLF:
    {
      SCRIPT_MSG(_T("载入语言文件: %") NPRIs _T("\n"),line.gettoken_str(1));
    ...
    return PS_ERROR;
    case TOK_P_WARNING:
      warning_fl(DIAGCODE_INTERNAL_HIDEDIAGCODE,_T("!警告: %") NPRIs,line.gettoken_str(1));
    return PS_OK;
    case TOK_P_ECHO:
    ...
    case TOK_UNINSTTEXT:
      {
        if (!cur_page) {
          ...
          if (cur_page_type != PAGE_UNINSTCONFIRM) {
            ERROR_MSG(_T("错误: UninstallText 只能在 PageEx uninstConfirm 页面中使用.\n"));
            return PS_ERROR;
          }
          cur_page->parms[0] = add_string(line.gettoken_str(1));
          cur_page->parms[1] = add_string(line.gettoken_str(2));
    ...
    case TOK_UNINSTSUBCAPTION:
      {
        ...
        SCRIPT_MSG(_T("UninstSubCaption: 页面:%d, 文本=%") NPRIs _T("\n"),w,line.gettoken_str(2));
      }
    return PS_OK;
    case TOK_WRITEUNINSTALLER:
    {
      if (uninstall_mode)
      {
        ERROR_MSG(_T("WriteUninstaller 仅在安装中有效, 不能在卸载中使用.\n"));
        PRINTHELP()
      }
      ...
      ent.offsets[4]=add_string(full.c_str());
      // ent.offsets[1], ent.offsets[2], ent.offsets[3], ent.offsets[5], ent.offsets[6] are set in CEXEBuild::uninstall_generate()
    ...
    case TOK_SECTION:
    {
      ...
      {
        ERROR_MSG(_T("错误: 声明了卸载段,但没有 NSIS_CONFIG_UNINSTALL_SUPPORT\n"));
        return PS_ERROR;
      }
#endif
    ...
    case TOK_FUNCTION:
      ...
      {
        ERROR_MSG(_T("Function: 函数名不能以 : 或 /. 开头\n"));
        PRINTHELP()
      }
      SCRIPT_MSG(_T("函数: \"%") NPRIs _T("\"\n"),line.gettoken_str(1));
#ifndef NSIS_CONFIG_UNINSTALL_SUPPORT
      if (!_tcsnicmp(line.gettoken_str(1),_T("un."),3))
      {
        ERROR_MSG(_T("错误: 声明了卸载函数,但没有 nsis_config_uninstall_support\n"));
        return PS_ERROR;
      }
#endif
    ...
    case TOK_SETCOMPRESS:
      ...
      if (build_compress==0 && build_compress_whole)
        warning_fl(DW_COMP_WHOLE_IGNORE_OFF, _T("遇到 'SetCompress off' 且处于整体压缩模式. 实际上被忽略."));
    ...
    case TOK_FILEBUFSIZE:
      ...
      {
        ERROR_MSG(_T("错误: FileBufSize: 无效的缓冲区大小 -- %d\n"),build_filebuflen);
        return PS_ERROR;
      }
      SCRIPT_MSG(_T("FileBufSize: %") NPRIs _T("mb (%d 字节)\n"),line.gettoken_str(1),build_filebuflen);
    return PS_OK;
#ifdef NSIS_CONFIG_COMPRESSION_SUPPORT
    case TOK_SETCOMPRESSIONLEVEL:
    {
      if (compressor == &lzmamt_compressor)
      ...
      if (build_compressor_set && build_compress_whole)
        warning_fl(DW_COMP_LEVEL_IGNORE, _T("SetCompressionLevel: 数据已在整体压缩模式下压缩. 实际上被忽略."));
    ...
    case TOK_SETCOMPRESSORDICTSIZE:
    {
      ...
      if (build_compressor_set && build_compress_whole)
        warning_fl(DW_COMP_DICT_IGNORE, _T("SetCompressorDictSize: 数据已在整体压缩模式下压缩. 实际上被忽略."));
    ...
    return PS_OK;
    case TOK_SETCOMPRESSORNUMTHREADS:
      build_compress_num_threads=line.gettoken_int(1);
      if (build_compress_num_threads < 1) build_compress_num_threads = get_logical_cpu_count();
      SCRIPT_MSG(_T("SetCompressorNumThreads: %d\n"),build_compress_num_threads);
    return PS_OK;
#else
    case TOK_SETCOMPRESSIONLEVEL:
    case TOK_SETCOMPRESSORDICTSIZE:
    ...
    case TOK_SUBCAPTION:
      {
        ...
        SCRIPT_MSG(_T("SubCaption: 页面:%d, 文本=%") NPRIs _T("\n"),w,line.gettoken_str(2));
      }
    return PS_OK;
    ...
    return PS_OK;
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_FILE.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif
        ...
        if (trim) try {
          ...
          if (trim) {
            ...
            if (td.GetItem(IDC_VERSTR)->sWidth > old_width)
              warning_fl(DW_ATTRIBUTE_OVERLONGSTRING,_T("BrandingText: \"%") NPRIs _T("\" 过长, 裁剪超出的标签"),str);
          }
          DWORD dwSize;
          dlg = td.Save(dwSize);
          ...
          ERROR_MSG(_T("错误的使用文本控件: %") NPRIs _T("\n"),CtoTStrParam(err.what()));
          return PS_ERROR;
        }
#else
        if (trim)
        {
          ERROR_MSG(_T("错误: BrandingText /TRIM* 在非Win32平台上被禁用.\n"));
          return PS_ERROR;
        }
#endif
    ...
    case TOK_DETAILSBUTTONTEXT:
      {
        if (!cur_page) {
          ...
          if (cur_page_type != PAGE_INSTFILES) {
            ERROR_MSG(_T("错误: DetailsButtonText 只能在 PageEx instfiles 页面中使用.\n"));
            return PS_ERROR;
          }
    ...
    case TOK_CALL:
      ...
      {
        ERROR_MSG(_T("Call 在卸载段中必须使用以 \"un.\" 开头的函数名.\n"));
        PRINTHELP()
      }
      if (!uninstall_mode && !_tcsnicmp(line.gettoken_str(1),_T("un."),3))
      {
        ERROR_MSG(_T("Call 不能在非卸载段中使用以 \"un.\" 开头的函数.\n"));
        PRINTHELP()
      }
#endif
      ent.which=EW_CALL;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_EXECUTE.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_SHELLEXECUTE.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_UNREGDLL:
      ...
      ERROR_MSG(_T("%") NPRIs _T(": (NSIS_SUPPORT_ACTIVEXREG)中不支持编译\n"),line.gettoken_str(0));
      return PS_ERROR;
    ...
    case TOK_RENAME:
#ifdef NSIS_SUPPORT_RENAME
      {
        ...
        if (!_tcsicmp(line.gettoken_str(1),_T("/REBOOTOK")))
        {
          ...
          ERROR_MSG(_T("错误: 指定了 /REBOOTOK, 但未定义 NSIS_SUPPORT_MOVEONREBOOT\n"));
          PRINTHELP()
#endif
        }
        else if (line.gettoken_str(1)[0]==_T('/'))
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_RENAME.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_MESSAGEBOX:
#ifdef NSIS_SUPPORT_MESSAGEBOX
      {
        ...
        if (line.getnumtokens()>a+1) SCRIPT_MSG(_T(" (在 %") NPRIs _T(" 时转到 %") NPRIs _T(")"),line.gettoken_str(a),line.gettoken_str(a+1));
        SCRIPT_MSG(_T("\n"));
      }
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_MESSAGEBOX.\n"),  line.gettoken_str(0));
    return PS_ERROR;
...
#ifdef NSIS_SUPPORT_CREATESHORTCUT
    {
      ...
      if (!s || ent.offsets[4] < 0 || ent.offsets[4] > CS_II_MAX)
      {
        ...
        {
          ERROR_MSG(_T("CreateShortcut: 无法解析图标索引\n"));
          PRINTHELPEX(cmdnam)
        }
      }
      ...
      if (line.getnumtokens() > 6 && *line.gettoken_str(6))
      {
        ...
        {
          ERROR_MSG(_T("CreateShortcut: 未知显示模式 \"%") NPRIs _T("\"\n"),line.gettoken_str(6));
          PRINTHELPEX(cmdnam)
        }
        ...
        if (*s)
        {
          ...
          if ((s[0] == _T('F')) && (s[1] >= _T('1') && s[1] <= _T('9')))
          {
            ...
            if (_ttoi(s+1) < 1 || _ttoi(s+1) > 24)
              warning_fl(DW_PARSE_LNK_HK, _T("CreateShortcut: F 键\"%") NPRIs _T("\" 超出范围"),s);
          }
          ...
          {
            c=s[0];
            warning_fl(DW_PARSE_LNK_HK,_T("CreateShortcut: 无法识别的热键 \"%") NPRIs _T("\""),s);
          }
          ent.offsets[4] |= ((c) << CS_HK_SHIFT) & CS_HK_MASK;
        }
      }
      SCRIPT_MSG(_T("CreateShortcut: \"%") NPRIs _T("\"->\"%") NPRIs _T("\" %") NPRIs _T(" 图标:%") NPRIs _T(",%d, nwd=%d, showmode=0x%X, hotkey=0x%X, comment=%") NPRIs _T("\n"),
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_CREATESHORTCUT.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_SETCTLCOLORS:
    {
      ...
      if (a == 2 && ctok == 5) {
        ERROR_MSG(_T("错误: SetCtlColors 需要 3 个参数, 但得到 4 个 参数\n"));
        return PS_ERROR;
      }
    ...
    case TOK_HIDEWINDOW:
    case TOK_ENABLEWINDOW:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_ENHANCEDUI_SUPPORT.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_CONFIG_ENHANCEDUI_SUPPORT
    ...
    case TOK_HIDEWINDOW:
    case TOK_BRINGTOFRONT:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_HWNDS.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_DELETE.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_RMDIR.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_SUPPORT_RMDIR
    case TOK_RESERVEFILE:
    case TOK_FILE:
    case TOK_RESERVESTUBFILE:
    case TOK_STUBFILE:
#ifdef NSIS_SUPPORT_FILE
      {
        ...
        switch (which_token)
        {
          ...
          break;
          case TOK_RESERVEFILE:
            which_token_str = _T("Reserve");
          break;
          case TOK_STUBFILE:
            which_token_str = _T("Stub");
          break;
          case TOK_RESERVESTUBFILE:
            which_token_str = _T("ReserveStub");
          break;
        }
        if (!_tcsicmp(line.gettoken_str(a),_T("/nonfatal")))
          fatal=false, a++;

        if ((which_token == TOK_RESERVEFILE || which_token == TOK_RESERVESTUBFILE) && !_tcsicmp(line.gettoken_str(a),_T("/plugin")))
        ...
        if (which_token == TOK_FILE && !_tcsicmp(line.gettoken_str(a),_T("/a")))
        {
          ...
          attrib=1;
#else
          warning_fl(DW_UNSUPP_STORE_FILE_ATT, _T("%") NPRIs _T("File /a 在非 Win32 平台上被禁用."),(which_token == TOK_FILE)?_T(""):_T("Reserve"));
#endif
          a++;
        }
        ...
        else if ((which_token == TOK_FILE || which_token == TOK_STUBFILE) && !_tcsnicmp(line.gettoken_str(a),_T("/oname="),7))
        {
          ...
          PATH_CONVERT(fn);
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
          set_datablock_mode(build_external_file == OFM_STUB && which_token == TOK_STUBFILE);
#endif
          ...
          if (!tf)
          {
            if (fatal)
            {
              ERROR_MSG(_T("%") NPRIs _T("文件: \"%") NPRIs _T("\" -> 未找到.\n"),which_token_str,line.gettoken_str(a));
              PRINTHELP()
            }
            else
            {
              warning_fl(DW_FILE_NONFATAL_NOT_FOUND, _T("%") NPRIs _T("文件: \"%") NPRIs _T("\" -> 未找到"),which_token_str,line.gettoken_str(a));
        ...
        while (a < line.getnumtokens())
        {
...
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
          set_datablock_mode(build_external_file == OFM_STUB ? (which_token == TOK_STUBFILE || which_token == TOK_RESERVESTUBFILE || reserveplugin) : 0);
#endif
          int v=do_add_file(fn, attrib, rec, &tf, NULL, (which_token == TOK_FILE || which_token == TOK_STUBFILE), NULL, NULL, excluded);
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
          set_datablock_mode(0);
#endif
          my_convert_free(fn);
          ...
          if (!tf)
          {
            ...
            {
              warning_fl(DW_FILE_NONFATAL_NOT_FOUND,_T("%") NPRIs _T("文件: \"%") NPRIs _T("\" -> 未找到."),which_token_str,t);
            }
          }
        }
      }
    ...
    case TOK_VERIFYEXTERNALFILE:
#ifdef NSIS_SUPPORT_FILE
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
      {
        if (line.getnumtokens() > 2) PRINTHELP()
        ent.which=EW_VERIFYEXTERNALFILE;
        ent.offsets[0]=add_string(line.gettoken_str(1));
        SCRIPT_MSG(_T("VerifyExternalFile: \"%") NPRIs _T("\"\n"),line.gettoken_str(1));
      }
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_EXTERNAL_FILE_SUPPORT.\n"), line.gettoken_str(0));
...
#else
    case TOK_COPYFILES:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_COPYFILES.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_GETDLLVERSIONLOCAL:
      {
        ...
        {
          ERROR_MSG(_T("%") NPRIs _T(": 从 \"%") NPRIs _T("\" 读取版本信息时出错\n"),cmdname,line.gettoken_str(1));
          return PS_ERROR;
        }
        ent.which=EW_ASSIGNVAR;
    ...
    case TOK_GETFILETIMELOCAL:
      {
        ...
        if (!flag)
        {
          ERROR_MSG(_T("GetFileTimeLocal: 从 \"%") NPRIs _T("\" 读取日期时出错\n"),line.gettoken_str(1));
          return PS_ERROR;
        }
#else
        struct stat st;
    ...
    case TOK_STRCMP:
    case TOK_STRCMPS:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_STROPTS.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_WRITEINISTR:
      ...
      SCRIPT_MSG(_T("WriteINIStr: [%") NPRIs _T("] %") NPRIs _T("=%") NPRIs _T(" 在 %") NPRIs _T("\n"),
    ...
    case TOK_WRITEINISTR:
    case TOK_READINISTR:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_INIFILES.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_GETFULLPATHNAME:
    case TOK_GETKNOWNFOLDERPATH:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_FNUTIL.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_GETDLLVERSION.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_GETFILETIME.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    case TOK_INTFMT: case TOK_INT64FMT:
    case TOK_INTCMP: case TOK_INTCMPU: case TOK_INT64CMP: case TOK_INT64CMPU: case TOK_INTPTRCMP: case TOK_INTPTRCMPU:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_INTOPTS.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    case TOK_READREGDWORD:
      {
        ...
        if (line.gettoken_str(3)[0] == _T('\\'))
          warning_fl(DW_PARSE_REGPATHPREFIX, _T("%") NPRIs _T(": 注册表路径名以 \'\\\', 开头时可能会出错"),line.gettoken_str(0));
    ...
    case TOK_WRITEREGDWORD:
      {
        ...
        ent.offsets[1]=add_string(line.gettoken_str(2));
        if (line.gettoken_str(2)[0] == _T('\\'))
          warning_fl(DW_PARSE_REGPATHPREFIX, _T("%") NPRIs _T(": 注册表路径名以 \'\\\', 开头时可能会出错"),cmdname);
        ent.offsets[2]=add_string(line.gettoken_str(3));
        if (which_token == TOK_WRITEREGSTR || which_token == TOK_WRITEREGEXPANDSTR)
        {
          SCRIPT_MSG(_T("%") NPRIs _T(": %") NPRIs _T("\\%") NPRIs _T("\\%") NPRIs _T("=%") NPRIs _T("\n"),
            cmdname,line.gettoken_str(1),line.gettoken_str(2),line.gettoken_str(3),line.gettoken_str(4));
          ent.offsets[3]=add_string(line.gettoken_str(4));
          ent.offsets[5]=ent.offsets[6]=REG_SZ;
          if (which_token == TOK_WRITEREGEXPANDSTR)
            ent.offsets[6]=REG_EXPAND_SZ;
        }
        if (which_token == TOK_WRITEREGBIN || multisz || which_token == TOK_WRITEREGNONE)
        {
          ...
          if (data_len < 0)
          {
            ...
            ERROR_MSG(_T("%") NPRIs _T(": %d 字节数据超出限制\n"),cmdname,sizeof(data));
            return PS_ERROR;
          }
...
#endif
          DB_OFFSET offset = add_db_data(data,data_len,&crc);
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
          set_datablock_mode(0);
#endif
          if (offset.quad_part == -1) return PS_ERROR;
          ent.offsets[3]=offset.low_part;
          ent.offsets[4]=offset.high_part;
          ent.offsets[5]=REG_BINARY, ent.offsets[6]=none?REG_NONE:multisz?REG_MULTI_SZ:REG_BINARY;
          ent.offsets[7]=FIX_ENDIAN_INT32(crc);
        }
        if (which_token == TOK_WRITEREGDWORD)
        {
          ent.offsets[3]=add_string(line.gettoken_str(4));
          ent.offsets[5]=ent.offsets[6]=REG_DWORD;
    ...
    case TOK_ENUMREGVAL:
      {
        ...
        if (line.gettoken_str(3)[0] == _T('\\')) warning_fl(DW_PARSE_REGPATHPREFIX,_T("%") NPRIs _T(": 注册表路径名称以 \'\\\', 开头时可能会出错"),line.gettoken_str(0));
    ...
    case TOK_ENUMREGKEY:
    case TOK_ENUMREGVAL:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_REGISTRYFUNCTIONS.\n"),  line.gettoken_str(0));
      return PS_ERROR;
    ...
    case TOK_PUSH:
    case TOK_EXCH:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_STACK.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_EXPANDENVSTRS:
    case TOK_READENVSTR:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_ENVIRONMENT.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_FINDNEXT:
    case TOK_FINDFIRST:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_FINDFIRST.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_FILEREADUTF16LE:
      if (!build_unicode)
      {
        ERROR_MSG(_T("错误: %") NPRIs _T(" 仅在构建 Unicode 安装程序时可用\n"),  line.gettoken_str(0));
        return PS_ERROR;
      }
      ent.which=EW_FGETWS;
      ent.offsets[0]=GetUserVarIndex(line, 1); // file handle
    ...
    case TOK_FILEWRITEWORD:
#endif
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_FILEFUNCTIONS.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_IFREBOOTFLAG:
    case TOK_SETREBOOTFLAG:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_REBOOT.\n"),  line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK_LOGSET:
    case TOK_LOGTEXT:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_LOG.\n"),  line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_CONFIG_LOG
#ifdef NSIS_CONFIG_COMPONENTPAGE
    ...
    case TOK_LOADANDSETIMAGE:
    {
      ...
      SCRIPT_MSG(_T("\n"));
      if ((lrflagsin & LASIM_LR) != lrflagsin) warning_fl(DW_PARSE_NUMBEROUTOFSPEC,_T("超出设定: \"%") NPRIs _T("\""),line.gettoken_str(tidx+2));
      if (fail) PRINTHELP();
    }
    return add_entry(&ent);
#else
    ...
    case TOK_VI_ADDKEY:
    {
      ...
      else
      {
        ...
        {
          ERROR_MSG(_T("%") NPRIs _T(": \"%") NPRIs _T("\" 不是有效的语言代码!\n"),line.gettoken_str(0), line.gettoken_str(1));
          return PS_ERROR;
        }
      ...
      // It is ok for us to use rVersionInfo as storage since VIProductVersion is required by VIAddVersionKey 
      {
        ...
        if (settingFileVer)
        {
          if (!validInput) 
          {
            ERROR_MSG(_T("错误: 格式错误 %") NPRIs _T(" 正确的格式为 X.X.X.X\n"),line.gettoken_str(0));
            return PS_ERROR;
          }
    ...
    case TOK_VI_SETPRODUCTVERSION:
    case TOK_VI_SETFILEVERSION:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_SUPPORT_VERSION_INFO.\n"),line.gettoken_str(0));
      return PS_ERROR;
    ...
    return add_entry(&ent);
#else
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_LOCKWINDOW_SUPPORT.\n"),line.gettoken_str(0));
    return PS_ERROR;
    ...
    case TOK__PLUGINCOMMAND:
    {
      ...
      {
        ERROR_MSG(_T("插件命令 %") NPRIs _T(" 与另一个目录中的插件冲突!\n"),command.c_str());
        return PS_ERROR;
      }

      tstring dllName = get_file_name(dllPath);
      DB_OFFSET data_handle = m_pPlugins->GetDllDataHandle(!!uninstall_mode, command);
      crc32_t crc = m_pPlugins->GetDllCRCValue(!!uninstall_mode, command);
      int ret;
      ...
      if (data_handle.quad_part == -1)
      {
        ...
        set_datablock_mode(build_external_file == OFM_STUB);
#endif
        ret=do_add_file(dllPath.c_str(),0,0,&files_added,tempDLL,2,&data_handle,&crc); // 2 means no size add
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
        set_datablock_mode(0);
#endif
        ...
        m_pPlugins->SetDllDataHandle(!!uninstall_mode, command, data_handle, crc);
      ...
      {
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
        ent.which = (build_external_file == OFM_STUB) ? EW_EXTRACTSTUBFILE : EW_EXTRACTFILE;
#else
        ...
        ent.offsets[2]=data_handle.low_part;
        ent.offsets[3]=data_handle.high_part;
        ent.offsets[4]=0xffffffff;
        ent.offsets[5]=0xffffffff;
        ent.offsets[6]=DefineInnerLangString(NLF_FILE_ERROR);
        ent.offsets[7]=crc;
        if ((ret=add_entry(&ent)) != PS_OK) return ret;
      }
      ...
      SCRIPT_MSG(_T("插件命令: %") NPRIs,funcname.c_str());
      ...
      SCRIPT_MSG(_T("\n"));
      if (nounloadmisused)
        warning_fl(DW_PLUGIN_NOUNLOAD_PLACEMENT, _T("/NOUNLOAD 必须放在任何插件参数之前. 除非您尝试使用的插件有一个参数 /NOUNLOAD, 否则您的操作有误"));
    ...
    case TOK__PLUGINCOMMAND:
    case TOK_INITPLUGINSDIR:
      ERROR_MSG(_T("错误: 指定了 %") NPRIs _T(" 但未定义 NSIS_CONFIG_PLUGIN_SUPPORT.\n"),line.gettoken_str(0));
    return PS_ERROR;
#endif //~ NSIS_CONFIG_PLUGIN_SUPPORT
...
#ifdef NSIS_SUPPORT_FILE
int CEXEBuild::do_add_file(const TCHAR *lgss, int attrib, int recurse, int *total_files, const TCHAR *name_override, int generatecode, DB_OFFSET *data_handle, crc32_t *pcrc, const set<tstring>& excluded, const tstring& basedir, bool dir_created)
{
       ...
       files_itr++)
  {
    ...
    if (add_file(dir, *files_itr, attrib, name_override, generatecode, data_handle, pcrc, which_token_str) != PS_OK) {
      return PS_ERROR;
    }
       ...
       dirs_itr++)
  {
    ...
    int res = do_add_file(new_spec_c, attrib, 1, total_files, NULL, generatecode, NULL, NULL, excluded, new_dir, created);
    if (res != PS_OK) {
      return PS_ERROR;
    }
  }
...
int CEXEBuild::add_file(const tstring& dir, const tstring& file, int attrib, const TCHAR *name_override, int generatecode, DB_OFFSET *data_handle, crc32_t *pcrc, const TCHAR *which_token_str) {
  ...
  if (hFile == INVALID_HANDLE_VALUE)
  {
    ERROR_MSG(_T("%") NPRIs _T("文件: 无法打开 \"%") NPRIs _T("\"\n"),which_token_str,newfn);
    return PS_ERROR;
  }
  ...
  {
    ERROR_MSG(_T("%") NPRIs _T("文件: 为 \"%") NPRIs _T("\" 创建内存映射失败\n"),which_token_str,newfn);
    return PS_ERROR;
  }
  ...
  if (generatecode&1)
    section_add_size_kb((int)((filesize+1023)/1024));
  if (name_override) SCRIPT_MSG(_T("%") NPRIs _T("File: \"%") NPRIs _T("\"->\"%") NPRIs _T("\""),which_token_str,filename,name_override);
  else SCRIPT_MSG(_T("%") NPRIs _T("File: \"%") NPRIs _T("\""),which_token_str,filename);
  if (!build_compress_whole)
    if (build_compress) SCRIPT_MSG(_T(" [compress]"));
  fflush(stdout);
  TCHAR buf[1024];
  UINT64 last_build_datablock_used=getcurdbsize();
  entry ent={0,};
  if (generatecode)
  {
#ifdef NSIS_CONFIG_EXTERNAL_FILE_SUPPORT
    ent.which=datablock_mode?EW_EXTRACTSTUBFILE:EW_EXTRACTFILE;
#else
    ent.which=EW_EXTRACTFILE;
#endif
    DefineInnerLangString(NLF_SKIPPED);
  ...
  crc32_t crc = 0;
  DB_OFFSET offset = add_db_data(&mmap, &crc);

  mmap.clear();

  if (offset.quad_part == -1) return PS_ERROR; 
  ent.offsets[2]=offset.low_part;
  ent.offsets[3]=offset.high_part;
  ent.offsets[7]=FIX_ENDIAN_INT32(crc);
  ...
  if (data_handle)
    data_handle->quad_part = offset.quad_part;

  {
    UINT64 s=getcurdbsize()-last_build_datablock_used;
    if (s) s-=FIRST_INT_SIZEOF;
    if (s != filesize)
    {
      FriendlySize us(filesize, GFSF_BYTESIFPOSSIBLE); // uncompressed installer size
      FriendlySize cs(s, GFSF_BYTESIFPOSSIBLE);        // compressed installer size
      SCRIPT_MSG(_T(" %u%") NPRIs _T(" / %u%") NPRIs _T("\n"),
        cs.UInt(),
        !_tcsicmp(cs.Scale(),us.Scale()) ? _T("") : cs.Scale(),
        us.UInt(),
        us.Scale());
    }
    else
    {
      FriendlySize us(filesize, GFSF_BYTESIFPOSSIBLE); // uncompressed installer size
      SCRIPT_MSG(_T(" %u%") NPRIs _T("\n"),us.UInt(),us.Scale());
    }
    if (getcurdbsize() > getcurdblimit())
    {
      FriendlySize ls(getcurdblimit(), GFSF_BYTESIFPOSSIBLE);
      ERROR_MSG(_T("%") NPRIs _T("文件: 添加失败,数据大于文件输出限制 (%u%") NPRIs _T(").\n"),which_token_str,ls.UInt(),ls.Scale());
      return PS_ERROR;
    }
  }
  ...
  if (generatecode)
  {
    if (build_datesave || build_overwrite>=0x3 /*ifnewer or ifdiff*/)
    {
      ...
      if (GetFileTime(hFile,NULL,NULL,&ft))
      {
        ...
        *fti -= *fti % 20000000; // FAT write time has a resolution of 2 seconds
        ent.offsets[4]=ft.dwLowDateTime, ent.offsets[5]=ft.dwHighDateTime;
      }
#else
      struct stat st;
      if (!fstat(fd, &st))
      {
        ...
        ent.offsets[4] = (int) ll, ent.offsets[5] = (int) (ll >> 32);
      }
#endif
      else
      {
        ERROR_MSG(_T("%") NPRIs _T("文件: 从 \"%") NPRIs _T("\" 获取文件日期失败\n"),which_token_str,newfn);
        return PS_ERROR;
      }
    }
    else
    {
      ent.offsets[4]=0xffffffff, ent.offsets[5]=0xffffffff;
    }
    ...
    ent.offsets[0] |= mb << 3;
    ent.offsets[6] = DefineInnerLangString(build_allowskipfiles ? NLF_FILE_ERROR : NLF_FILE_ERROR_NOIGNORE);
  }
