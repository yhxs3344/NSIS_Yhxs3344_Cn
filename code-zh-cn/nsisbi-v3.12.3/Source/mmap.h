...
{
  public:
    typedef UINT64 maxfilesizetype;
    static inline UINT64 getmaxoffset() { return (UINT64)0x7fffffffffffffff; }

    virtual void resize(UINT64 newlen)=0;
    virtual UINT64 getsize() const=0;
    virtual void *get(UINT64 offset, size_t size) const=0;
    virtual void *get(UINT64 offset, size_t *size) const=0;
    virtual void *getmore(UINT64 offset, size_t size) const=0;
...
private:
    int internalsetfile(HANDLE hFile, UINT64 dwSize);
#else
    bool setfile(FILE*hFile, UINT64 size);
private:
    int internalsetfile(int hFile, UINT64 dwSize);
#endif
...
public:
    ...
    * 
    * @param newsize The new size of the file.  Limited to 63-bits.
    */
     ...
     */
    void *get(UINT64 offset, size_t size) const;
     ...
     */
    void *get(UINT64 offset, size_t *sizep) const;
     ...
     * @param size The size of the memory map window.
     */
    void *getmore(UINT64 offset, size_t size) const;
  ...
  private:
    ...
    FILE *m_hFile;
    int m_hFileDesc;
    mutable UINT64 m_iMappedSize;
#endif
    mutable void *m_pView;
    mutable UINT64 m_iSize;
    BOOL m_bReadOnly;
    BOOL m_bTempHandle;
...
class MMapBuf : public IGrowBuf64, public IMMap
{
  ...
  public:
    ...
    UINT64 add(const void *data, int len);
    void setro(BOOL bRO);
    void resize(UINT64 newlen);
    UINT64 getsize() const;
    UINT64 getlen() const;
    void *get() const;
    void *get(UINT64 offset, size_t *sizep) const;
  ...
  protected:
    static inline unsigned int getmodethreshold() { return 16 << 20; }
