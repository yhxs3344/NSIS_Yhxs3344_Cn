...
public:
  void testMMapFile() {
    ...
    CPPUNIT_ASSERT_EQUAL( BUF_SIZE, (int)mmap.getsize() );
  ...
  void testMMapBuf() {
    ...
    CPPUNIT_ASSERT_EQUAL( tmpint, (int)mmap.getsize() );
    mmap.resize(tmpint = threshold + 1);
    CPPUNIT_ASSERT_EQUAL( tmpint, (int)mmap.getsize() );
    mmap.resize(tmpint = threshold - 1);
    CPPUNIT_ASSERT_EQUAL( tmpint, (int)mmap.getsize() ); // We don't care if it does not go back to GrowBuf but the size still has to be correct
  }
