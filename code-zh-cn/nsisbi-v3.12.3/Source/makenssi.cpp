...
#define NSIS_COPYYEARS _T("1999-2026")
...
{
  if (!g_dopause) return;
  if (g_display_errors) _ftprintf(g_output,_T("制作完成 - 按回车键后关闭..."));
  fflush(stdout);
  int a;
...
void quit()
{
  if (g_display_errors) 
  {
    PrintColorFmtMsg_WARN(_T("\n注意: 您的临时目录中可能遗留了一个或两个（较大的）过时临时文件 ")
         _T("(通常仅发生在 Windows 9x 上).\n"));
  }
  exit(1);
}
...
{
  _ftprintf(g_output,_T("MakeNSIS %") NPRIs _T(" - 版权所有 ") NSIS_COPYYEARS _T(" 贡献者\n")
         _T("许可证细节请参见 COPYING 文件.\n")
         _T("致谢信息请参见用户手册.\n\n"), NSIS_VERSION);
  fflush(g_output);
}
...
{
  _ftprintf(g_output,_T("版权所有 (C) ") NSIS_COPYYEARS _T(" Nullsoft 及贡献者\n\n")
       _T("本许可证适用于 NSIS 软件包中的所有内容，除非另有说明\n")
       _T("敬告.\n\n")
       _T("本软件遵循“概不保证”的原则，没有其它任何的说明和暗示.\n")
       _T("在任何情况下\n")
       _T("作者均不对因使用本软件而产生的任何损害承担责任.\n\n")
       _T("允许任何人将本软件用于任何目的，包括商业应用\n")
       _T("并允许自由修改和重新分发，但须遵守\n")
       _T("下列条款:\n")
       _T("  1. 不得歪曲本软件的来源；\n")
       _T("     您不得声称您编写了原始软件.\n")
       _T("     如果您在产品中使用本软件，产品文档中的致谢信息将被 appreciated,\n")
       _T("     但这并不是必须的.\n")
       _T("  2. 修改后的源代码版本必须明确标记为修改版本\n")
       _T("     且不得被歪曲为原始软件.\n")
       _T("  3. 不得从任何源代码分发中删除或修改本声明.\n\n")
       _T("除本协议外, 附带的压缩模块适用不同的许可证\n")
       _T("详情请参见 COPYING 文件.\n"));
  fflush(g_output);
}
...
static void print_usage()
{
  _ftprintf(g_output,_T("用法:\n")
         ...
         _T("\n")
         _T("选项:\n")
         _T("  ") OPT_STR _T("/CMDHELP [项] 输出关于“项”的帮助信息, 或列出所有命令\n")
         _T("  ") OPT_STR _T("/HDRINFO 输出编译 makensis 时使用的配置信息\n")
         _T("  ") OPT_STR _T("/LICENSE 输出 makensis 软件许可协议\n")
         _T("  ") OPT_STR _T("/VERSION 输出 makensis 版本并退出\n")
         //_T("  ") OPT_STR _T("本帮助信息\n")
#ifdef _WIN32
         _T("  ") OPT_STR _T("/Px 设置编译器进程的优先级, 其中 x=5 实时,4 高,\n")
         _T("  ")         _T("  3 高于正常,2 正常,1 低于正常,0 空闲\n")
#endif
         _T("  ") OPT_STR _T("/Vx 详细程度, 其中 x=4 全部,3 信息、警告和错误,2 警告和错误,1 仅错误,0 无输出\n")
         _T("  ") OPT_STR _T("WX将警告视为错误\n")
         _T("  ") OPT_STR _T("/Ofile 指定记录编译器输出的文本文件 (默认为 标准输出)\n")
         _T("  ") OPT_STR _T("LAUNCH 执行生成的安装程序\n")
         _T("  ") OPT_STR _T("/PAUSE 执行后暂停\n")
         _T("  ") OPT_STR _T("/NOCONFIG 禁止包含 <makensis.exe 所在路径>") PLATFORM_PATH_SEPARATOR_STR _T("nsisconf.nsh\n")
...
#endif
         _T("  ") OPT_STR _T("/PPO [SAFE]PPO 预处理到标准输出/文件\n")
         _T("  ") OPT_STR _T("/Ddefine[=value] 为脚本文件 定义符号 \"define\" [赋值为 value]\n")
         _T("  ") OPT_STR _T("/Xscriptcmd 在脚本中执行 scriptcmd (例如 \"") OPT_STR _T("XOutFile inst.exe\")\n")
         _T("  ")         _T("  参数按照先后顺处理 (") OPT_STR _T("Ddef ins.nsi != ins.nsi ") OPT_STR _T("Ddef)\n")
         _T("\n")
         _T("对于脚本文件名, 您可以使用 - 来从标准输入读取\n")
#ifdef _WIN32
         _T("您也可以使用 - 作为选项字符: -PAUSE 以及 /PAUSE\n")
#endif
         _T("您可以使用双横线 -- 结束选项处理 -- -ins.nsi\n"));
  fflush(g_output);
}
...
static void print_stub_info(CEXEBuild& build)
{
  ...
  {
    _ftprintf(g_output,_T("第一个头文件大小为 %lu 字节.\n"),(unsigned long)sizeof(firstheader));
    _ftprintf(g_output,_T("主头文件大小为 %lu 字节.\n"),(unsigned long)build.get_header_size());
    _ftprintf(g_output,_T("每个区段大小为 %lu 字节.\n"),(unsigned long)sizeof(section));
    _ftprintf(g_output,_T("每个页面大小为 %lu 字节.\n"),(unsigned long)sizeof(page));
    _ftprintf(g_output,_T("每条指令大小为 %lu 字节.\n"),(unsigned long)sizeof(entry));
...
static int process_config(CEXEBuild& build, tstring& conf)
{
  ...
  {
    build.INFO_MSG(_T("正在处理配置文件: %") NPRIs _T("\n"),conf.c_str());
...
static int change_to_script_dir(CEXEBuild& build, tstring& script)
{
  ...
  if (!dir.empty()) 
  {
    build.SCRIPT_MSG(_T("正在更改目录为: \"%") NPRIs _T("\"\n"),dir.c_str());
    if (_tchdir(dir.c_str()))
    {
      build.ERROR_MSG(_T("更改目录到 \"%") NPRIs _T("\"时出错\n"),dir.c_str());
      return 1;
    }
    build.SCRIPT_MSG(_T("\n"));
  }
...
static inline int makensismain(int argc, TCHAR **argv)
{
  ...
  if (!stdoutredir) PrintColorFmtMsg_WARN(_T("打开输出日志文件用于写入时出错！将使用 stdout 标准输出.\n"));
...
#else
  const TCHAR*const badnonwinswitchfmt=OPT_STR _T("%") NPRIs _T(" 在非 Win32 平台上被禁用.");
  if (hostnotifyhandle)
  ...
  while (argpos < argc)
  {
    ...
    else if (IS_OPT(argv[argpos]) && _tcscmp(argv[argpos], _T("-")) && !in_files)
    {
      ...
      else if (!_tcsicmp(swname,_T("INPUTCHARSET")) || !_tcsicmp(swname,_T("ICS")))
      {
        ...
        if (NStreamEncoding::UNKNOWN == cp)
        {
          if (_tcsicmp(argv[argpos], _T("AUTO")))
            build.warning(DW_CMDLINE_BAD_INPUTENC, OPT_STR _T("INPUTCHARSET: 忽略无效字符集 %") NPRIs , argv[argpos]);
               ...
               argv[argpos][2] >= _T('0') && argv[argpos][2] <= _T('5') && !argv[argpos][3])
      {
        ...
        if (p == 5) build.warning(DW_CMDLINE_HIGHPRIORITY,_T("makensis 正在以实时优先级模式运行!"));
#else
      ...
      else if (S7IsChEqualI('d',swname[0]) && swname[1])
      {
        ...
        TCHAR *s=_tcsdup(p),*v;
        build.INFO_MSG(_T("命令行定义: \"%") NPRIs _T("\"\n"),p);
        v=_tcsstr(s,_T("="));
        if (v) *v++=0;
...

      {
        ...
        else
        {
          ...
          if (!strm.OpenFileForReading(nsifile.c_str(),inputenc))
          {
            ...
            {
              nsifile = argv[argpos];
              build.ERROR_MSG(_T("无法打开脚本 \"%") NPRIs _T("\"\n"),nsifile.c_str());
              return 1;
            }
          }
        ...
        build.INFO_MSG(_T("正在处理脚本文件: \"%") NPRIs _T("\" (%") NPRIs _T(")\n"),nsifile.c_str(),bufcpdisp);
  ...
  {
    _ftprintf(g_output,_T("\n已处理 "));
    if (files_processed) _ftprintf(g_output,_T("%d 个文件%") NPRIs _T(", "),files_processed,files_processed==1?_T(""):_T("s"));
    if (cmds_processed) _ftprintf(g_output,_T("%d 条命令行命令%") NPRIs _T(", "),cmds_processed,cmds_processed==1?_T(""):_T("s"));
    _ftprintf(g_output,_T("正在写入输出 (%") NPRIs _T("):\n"),build.get_target_suffix());
    fflush(g_output);
  }
